...
- At the next run's start you may equip up to `vaultSlots =
  ...
  included); withdrawing removes a relic from the Vault. Equipping raises
  the run's **minimum ascension** by one per relic worn —
  `minAscension = minAscensionFor(equipped)` in `screens/vault.ts`, which
  the ascension stepper clamps to and says why on screen. The clamped value
  is what goes into `RunConfig.ascension`; `game/sim/run.ts` is unchanged.
...
long: **exactly one plane is pixelated — and the ground it stands on.** The
actor plane and the FLOOR draw hard pixels on the actors' own `ACTOR_SCALE`
cell; FAR, MID and NEAR keep their blur, which is the depth-of-field split
the rule was really protecting. Light, particles, fog, UI and text all
render smooth at 720p.
...
**Two kinds of text.** Bitmap `FONT_HD` for what belongs to the world and the
arcade, and ONLY where it stands alone: damage pops (`TEXT_POP 3`, crits 4)
and the title logo — never below scale 2, and never in the same box as HUD
copy. Card titles, door labels, the room card's name and a pact's name are UI
the player READS, so they render in the HUD face at `HUD_TITLE = 28`
(`HUD_TITLE_H = 33`) in the card's own colour. The title band survives as the
head of a card; it is now one voice in one box instead of two. Everything
...
`#ffa64a` (the crypt key light's own amber — EVERY screen title, every
primary button, rings, prompts, the ribbon's caret). Where a screen's own
family still has to be said, it is said in a WORD, not in the plate:
`drawBanner(ctx, text, accentWord?, accentColor?)` draws the named word of an
otherwise-amber title in its own colour — SUMMON cyan, SHRINE violet, ALTAR
gold, HEAL green — so the colour identifies the thing it names. `ACCENT_COOL` `#4fc4de` (Tide's orb — RARE, wards, ATB) and
`ACCENT_HP` `#7fd45c` (Gale's green — every HP bar, in battle, on the map and
on the party columns alike), which SHIFTS toward `C_DEBUFF` as the bar empties:
`ACCENT_HP` at or above `HP_DANGER_FROM = 0.35`, walking to the debuff rose by
`HP_DANGER_FULL = 0.1`, quantised to twelve baked steps (`hpColor`) so a bar
drawn several times a frame builds no strings. Pure yellow, pure cyan
and pure green are retired. Two buttons exist. The PRIMARY action is a lit
patch of the floor, and the patch is a LOW ELLIPSE: the ink is a radial
gradient solid to `PRIMARY_FEATHER = 0.62` of its radius and gone by 1.0,
drawn under a vertical squash, so it has no straight edge on any side —
feathering only the ends left the top and bottom as hard horizontal rules and
what the player saw was a lit BAND. Over it the key's own colour is ADDED
(`'lighter'`) as a radial centred on the patch's crown, which is what says
"light lands here" now that the straight lip is gone. FOCUS is the same
glow-and-lift the rest of the game uses and is the only state that bleeds
outside the patch. It is still one gold on every screen. The SECONDARY is
text on a short quiet plate — the label's own width plus air, never its
whole hit rect — with a focus underline; the 1-px-bordered rounded rectangle
is retired. A
...
| hero panels | x 976–1256 (the party's own side), three of `PANEL 280×104` at y 96/212/328; `PANEL_PAD = 7`, rows NAME 22 · HP 22 · ATB 6 · STATUS 28, gaps 4. A panel is a **plate**: a top-down ink gradient `PANEL_TOP_ALPHA = 0.35` → 0 (0.46 for the actor on turn), no border unless it is the actor on turn — FOCUS is a `FOCUS_GLOW_BLUR`-wide bleed of the key colour drawn UNDER the plate (`focusGlow`, clipped even-odd out of the plate's own footprint so it is light spilling from behind and never a wash inside) plus a value lift (`focusLift`: a `FOCUS_LIFT_H = 26` accent wash down from the top edge, a 1-px lip where the light meets it, and a `FOCUS_BODY_ALPHA = 0.07` tint across the whole body so focus survives a greyscale reduction). A CHOSEN-but-not-focused thing — the party column the two buttons will act on, the draft offer already picked — carries the same light at `FOCUS_CHOSEN = 0.55`, not a border. The pause overlay's secondaries are the one exception the contract already names: over a 0.66 dim with no lit world behind them a borderless plate dissolves, so they keep their border and take the glow on top. The NAME row closes with a 20-px `ELEMENT_GLYPH` pictogram, never a saturated chip; HP is the number right-aligned with a 4-px rule **the width of that number** over a trough tinted to the actor's element; ATB is the 2-px line and the ONLY attack bar drawn on a hero. The STATUS shelf is **not drawn when empty** and `PANEL_H` becomes the drawn ceiling, not the drawn height — the registered rect stays 280×104. Up to six 28-px status pictograms; past six, five and a "+N". Tap = target while a target prompt is open, else inspect |
| stage | x 24–1160 (one panel column instead of two walls): the party at (532, 380) · (632, 448) · (732, 516) feet facing left, `DIAG_DX 100 / DIAG_DY 68`; the pack on its own shorter step `PACK_DX 46 / PACK_DY 68` at (290, 380) · (336, 448) · (382, 516), so neighbouring packmates overlap by 9.5 and 12 px — 17 % and 21 % of the 50 / 61 / ~55 px the crypt pack actually DRAWS, measured by per-column edge energy, not of the 68-px hurtbox; a pack of two at (352, 404) · (398, 472); a lone boss at (410, 490). The anchor-to-anchor gap between the ranks is 532 − 382 = **150 px = 11.72 %** of the width, from 21 %. `rankFrontier()` is the midpoint between the pack's last foot and the party's first — 457 — and `spriteCellX` takes it as the outer bound of both ranks' cells, so the two halves of the stage are cut at one line: the pack owns `STAGE_X0`..frontier, the party frontier..its own last halfW, and **no point on the stage is owned by two actors**. Points owned by NOBODY are the design and remain: a cell is its actor's hurtbox grown to `TAP_MIN`, never a full-height column, so the floor between two ranks' rows belongs to neither. **No gauges on the ground**: an actor carries a 3-px HP hairline (`HP_HAIRLINE_H`) tucked into its contact shadow, `HP_HAIRLINE_SPAN = 0.62` of the foot span and never wider, shown only while HP is short or for `HP_HAIRLINE_HOLD = 1.5 s` after a change; ATB is drawn in the hero panels and in the ribbon's order only. ≤ `STATUS_ABOVE_MAX = 4` status pictograms above the head (the REAL silhouette top from `actorHitRect`, not a fixed 0.88 × `ACTOR_W`; then 3 + "+N"); pops at head + 64, rising at most `POP_RISE_MAX = 26` and clamped out of the ribbon, the safe inset and the panel column — except a plate-bearing enemy's own pop, which `popYFor` pushes above its plate instead. Enemies are targeted through sprite cells whose x-span `spriteCellX` splits at the midpoint between neighbouring feet, a Voronoi cut leaving no strip of the stage ambiguous between two packmates |
...
| cards | `CARD_W = 384` × `CARD_H = 472` at `CARD_Y = 88`, x 40/448/856 — the band runs 88..560 and a card ROW FILLS it. Every card in a row is measured (`relicCardParts`) so the row can be laid out as one, the head group sits at the band's top, the set and sigil groups stay anchored to its foot, and a short card carries its slack as air between its own blocks rather than as a dead strip under all three. A single card that is measured rather than filled (the room card, the SHRINE's pact card) takes a floor — `ROOM_CARD_MIN_H = 300` — so the gap under it stays inside the 100-px bar. The set block and the sigil are the card's foot group, anchored to the bottom edge. The who-wears-it columns are measured together the same way. The who-wears-it column carries the candidate's portrait chip AND the candidate themselves idling at `ACTOR_SCALE`, clipped to the column, in place of the word EMPTY, or four of `CARD_W_FOUR = 284` × 472 at x 48/348/648/948 under HASTE; the room-to-room card is the middle slot with `CONTINUE = (448, 568, 384, 96)` — one `FOOT_ROW_Y = 568` that every foot seat is built from; the who-wears-it row is its own grid: `WEAR_BTN 280×96` at `WEAR_X = 40/344/648/952`, `WEAR_Y = 568` (the fourth is decline), each candidate's current piece and compare line in the three card slots. The row closes at 664, clear of the controls hint at 678 |
| doors | DESCEND and ANOTHER LAP, 520×260 at `DOOR_X = 96 / 664`, `DOOR_Y = 340`, their two lines centred under the title band (`DOOR_Y + 26 + TITLE_H + 78 + 40k`) — the doors are their own foot row, so at 200×320 they left 152 px of bare floor between their feet and the controls hint, the worst dead band on any run screen; at 260×340 their feet close at 600, 72 px above it |
| inspect | `INSPECT = (160, 56, 960, 480)` — 58 % of the area for the same content, so the dimmed crypt around it does the framing instead of more black. Name at `HUD_LARGE` (184, 72); six rows at `INSPECT_ROW_Y = 120 + 58 × i`, each on its own plate — slot pictogram, title, the main line right-aligned, and all four substats on the row's second line; an empty row says "nothing worn" and the mains that slot can roll. The WHO column (x 184, w 288) carries the element mark, HP with its rule, the character idling at `ACTOR_SCALE` and the eight derived stats two to a row. Set bonuses in `SET_BAND = (184, 468, 880, 62)` at `SET_LINE_Y = 472 / 494 / 516`, with the member's three SKILLS and their cooldowns on the same band from `SET_BAND.x + 468`. The overlay is `dimScene(0.62)` with the panel over it at plate alpha **0.92**, drawn with the battle HUD (panels, ribbon, log, command list, and the stage plane's own hairlines and head pictograms) suppressed beneath it, so no panel, number or log line ghosts through; `BACK` keeps its own seat at (1040, 552, 192, 96), below the panel's right end over the dim, also bound to B |
...
| map | `MAP_NODE = 96` at `MAP_X = 88 + 208 × stage`, `MAP_Y = 168 + 144 × row`; six columns (`STAGE_SIZES` then the implicit BOSS at stage 5) at x 88 / 296 / 504 / 712 / 920 / 1128, three rows at y 168 / 312 / 456 with a short stage centred over them (2 nodes → rows 0 and 2, 1 node → row 1). Every node registers, group `map`: the offered successors indexed **in `offeredIdxs` order** (that index IS the route answer) and every other node `disabled` — focusable, so the player can read what they are turning down, never activatable. The act and score band y 24–120 carries ACT n/6 · LAP · A-level · SCORE, the party's HP pips from `MAP_PIP_X = 336` (`MAP_PIP_W = 190` per member), `MAP_PARTY = (936, 24, 216, 96)` (index 10 — opens the party screen read-only, also bound to B) and the pause icon. Routing cannot be declined |
| party | three member columns in the card slots, each with a head at y 56 (portrait, name, element, HP bar, a star on the leader, the AWAKENED mark) and six `PARTY_ROW = 64` slot rows from y 128 in `SLOTS` order as its `index`; a row registers its DRAWN 384 × 64 rect. **A row's action here is "select this column"** — empty rows included — because that is what the two conditional buttons act on; relics are never moved on this screen (the who-wears-it row at a card screen is the only mover). Set bonuses in `PARTY_SET_BAND = (640, 512, 304, 136)`, with its own label row at y 512. `PARTY_SWAP = (40, 568, 280, 96)`, `PARTY_LEADER = (344, 568, 280, 96)` (acts on the focused column; `disabled` outside the draft, a SUMMON, a REST and the ALTAR — i.e. precisely when a `leader()` decision is open) and `PARTY_BACK = (952, 568, 280, 96)`, group `party` — the six slot rows carry index 0–5, `PARTY_SWAP` index 8, `PARTY_LEADER` index 9, `PARTY_BACK` index 10 — BACK also bound to B. With a leader decision open BACK reads "KEEP <name>" and answers with the seat as it stands, which is that decision's own default. A face that reuses the columns registers them under its own prefix and group (`forge` / `altar` / `rest` / `bank` / `swap`) with the same geometry |
| act clear | **act clear.** A held tableau between an act's last reward and the next act's ROUTE. `screens/run.ts` raises it the moment the seam's own `act` steps forward with the run still going (`RunPhase` gains `ACT_CLEAR`; `RunState` gains `actCleared` and `actClearedBiome`) and drops it on `clearAct()`. Like the room card it is PRESENTATION over a pending the seam already holds — it asks nothing and answers nothing. `screens/end.ts` draws it: the whole party in the end screens' three seats (survivors idling, the fallen in their dead pose) under the same warm pool, over a lighter `BEAT_DIM = 0.38` because the run goes on, with ACT n CLEARED, the biome's name, the score and the room count. One full-canvas region (`act-clear`, index 0) so a tap anywhere goes on, A activates it, B skips it, and it walks on by itself after `ACT_CLEAR_HOLD = 3.4 s` — so no screen can swallow a run and a driver that does not know the beat only has to wait it out |
| end screens | The WHOLE party stands in the frame at (896, 424) · (986, 492) · (1076, 560), drawn inside the scene pass so the biome's key light and rim reach them — survivors idling, the fallen in their `dead` pose. A GAME OVER has no survivors by definition, so the old rule left that screen as four centred lines and a button over ground measuring p50 15.8 where the party would stand. A cached warm pool is laid back over `dimScene(0.5)` so they still read as lit under the terminal overlay. A biome that declares a **`sky` body** (`SkyLight` on its `BiomeLook` — the marsh's moon, the ruins' sun, the spire's afterglow) has it re-applied additively in `renderPost` **after** the grade, as a hot core inside a long falloff, so the brightest object in the sky dims *proportionally* under that overlay instead of flattening into a grey disc with a coloured ring. GAME OVER's RETRY and VICTORY's CONTINUE reuse `CONTINUE` as the **primary** button (lit borderless plate in the one amber every primary wears); the verdict's own colour lives in the WORD above it — VICTORY amber, GAME OVER in the debuff rose; the act-6 VICTORY shows the doors row; score and act at `TEXT_LABEL` centred at y 120; the title's START, the Home-Screen hint and the rotate prompt are screen-level |
| draft | `DRAFT_CARD = 284 × 136` in the four-card columns 48 / 348 / 648 / 948 at `DRAFT_Y = 88 / 240 / 392` (twelve slots), row-major, with a short row centred on the frame at `DRAFT_PITCH = 300` (three options at x 198 / 498 / 798; even counts fall on the four columns 48 / 348 / 648 / 948), group `draft`, index = slot; the pick is confirmed by `CONTINUE` (index 10, `disabled` until something is picked). The grid row(s) the cards did NOT use become a **detail strip** for the focused option, `1184` wide at x 48 and `DETAIL_BLOCK_H + 2 × DETAIL_PAD` = 168 tall — the block it holds plus its air, not the rows the cards left over — at y 392 when the cards fill two rows and, when they fill one, centred in the space between the card row's foot and the DRAWN primary seat — `CONTINUE.y + PRIMARY_SEAT_INSET`, 588, not the hit rect's 568, because the slab is 56 tall inside a 96-px rect — so the two halves are 98 px each and the strip takes a `base` so its own ink reaches its own foot; clamped so its foot never comes closer than `DETAIL_FOOT_GAP = 12` to `CONTINUE` — three columns at +20 / +380 / +800 — which opens the FOCUSED option out (cooldowns, targets, base stats, and for a SUMMON whether the recruit is favoured against the coming act's dominant element). The height the cards leave unused goes back to the frame as air. A SUMMON with a full party adds `TAKE THE EPIC` on the fourth column of the top row (`(948, 88, 284, 136)`, index = option count) and, once an offer is picked instead, a who-goes step on the party columns. Declining is `DECLINE` on the wear grid's fourth seat (`WEAR_X[3]`, `WEAR_Y`, index 11) and B — a SUMMON and a REBRAND may decline, **the draft may not** (B is inert there and slot 0 is pre-picked). One grid, three payloads: the draft, a SUMMON's offers, the FORGE's REBRAND set list |
| skip | `SKIP = CONTINUE` registered under every card row (SKIP / WALK PAST / DECLINE / FULL HEAL), group `cards`, index = card count, also bound to B; FORGE's three modes and walk-past use the WEAR grid after the relic is picked on the party columns. A SHRINE is the same pair one card wide: the pact card in the middle slot is ACCEPT (index 0) and this seat is WALK PAST (index 1) |
| shrine | the pact as one card in the middle card slot — `(448, 88, 384, 472)`, group `cards`, index 0 (an exception to the `cards` row above: the pact card registers its drawn, height-fitted plate, grown to `TAP_MIN` by the registry; the 472 band is not a target — e.g. `(448, 198, 384, 300)`, its floor, for a one-line blurb) — name ≤ `NAME_MAX_PACT`, a CURSE line, a BOON line and the blurb ≤ `BLURB_MAX` wrapped to ≤ `BLURB_LINES_MAX` lines. Tapping the card accepts; `SKIP` (index 1) and B walk past, which is the default. Pacts already taken ride as chips from `(40, 128, 300 × 44, gap 8)`. No mend either way |
| forge | three steps on geometry that already exists. (1) the relic: the party columns, group `forge`, rows enabled only where `worn` has an entry that `forgeOptions` names. (2) the mode: the WEAR grid — `WEAR_X[0]` LEVEL +n (`disabled` when the relic is capped), `[1]` RECAST, `[2]` REBRAND (`disabled` when `rebrandSets` is empty), `[3]` WALK PAST. (3) the detail: RECAST offers the relic's four substat lines as `(448, 200 + 72 × i, 384 × 64)`, index 0–3, `disabled` where the line is empty; REBRAND borrows the draft grid (≤ 5 sets). B steps back one level and, from the first, walks past — the default |
| altar | the party columns in whole-column mode — `(CARD_X[m], 56, 384, 456)`, group `altar`, index = member — enabled only for the un-awakened, each showing `def.awakening.name` and what it grants; `CONTINUE` (index 8) confirms and is `disabled` until a column is picked. The ALTAR **cannot be declined**: B is inert and the seam awakens `candidates[0]` if the screen is ever left |
| rest | the party columns in whole-column mode, group `rest`, enabled only for `sharpenCandidates`; a column notes the heal it would give, and its rows tag the ≤ `SHARPEN_RELICS` uncapped relics a SHARPEN would touch (`+1`) and the ones it would not (`CAPPED`). `CONTINUE` reads FULL HEAL (index 8) and is the default, also bound to B |
| vault | the twelve `DRAFT_CARD` slots are the Vault grid, group `vault`, index = slot. EQUIP: a chip is `disabled` once its `Slot` is spoken for or `vaultSlots` picks are made, `CONTINUE` (index 19) reads BEGIN THE RUN, and the ascension stepper is ONE control on the foot row left of it — `ASC_DOWN = (40, 568, 96, 96)` (index 20) · `ASC_PLATE = (144, 584, 200, 64)`, the level and its reason · `ASC_UP = (352, 568, 96, 96)` (index 21) — each arrow `disabled` at its own limit (the floor is `minAscensionFor(equipped)`, the ceiling `max(floor, unlocked)`); B clears the picks. BANK: the worn relics are picked on the party columns (group `bank`, at most `n`), `CONTINUE` (index 8) commits, and **only** when `vault + take` overflows `VAULT_SIZE` does a second face open on the same grid to pick the drops — `CONTINUE` there is never `disabled` on the drop count (the seam trims whatever is still over the cap, lowest level first, so an under-filled pick is completed for the player, not blocked) — with B stepping back |
| ascension | chosen on the Vault's EQUIP face before the run, never during it: the stepper above, a band at `VAULT_BAND = (24, 24, 1232, 56)` naming the level and what it adds, and a floor the screen enforces and says out loud ("A2 minimum — one per Vault relic worn") — `minAscensionFor(equipped)`, one per Vault relic worn (see *The Vault*). The chosen level is `RunConfig.ascension` and is immutable for the run |
...
- **Vault + three kindled relics, act-1 clear ≤ 97 %** — triggered, and
  ...
  file ownership and outside a BUG-FIXES-only edit to `run.ts`. **Landed
  with the phase-5/6a Vault screen**: `minAscensionFor(equipped)` in
  `screens/vault.ts` is `Math.min(ASCENSION_MAX, equipped.length)` — one
  per Vault relic worn (capped at `ASCENSION_MAX`), not only the kindled
  ones — the EQUIP face clamps its stepper to it and says
  why, and `main.ts` passes the clamped value in as `RunConfig.ascension`.
  `game/sim/run.ts` and `RunConfig` are untouched: ascension is still a
  fixed input the player picks before the run. Re-measure the guard
  (`node sim/run.mjs --runs 5000 --vault 3 --policy balanced`) at the floor
  the screen now imposes — A3 — rather than at A0.
