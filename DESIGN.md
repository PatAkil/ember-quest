...
borderless plate with a soft key-coloured glow, the SECONDARY is text on a
short quiet plate — the label's own width plus air, never its whole hit rect —
with a focus underline; the 1-px-bordered rounded rectangle is retired. A
...
| cards | `CARD_W = 384` × 440 at `CARD_Y = 88`, x 40/448/856 — 440 is the **hit** height; every card in a row is measured first (`relicCardParts`), the row takes the TALLEST of those heights, and that one height is centred in the band for all of them (a relic card was 45 % empty below the set line, the room card 62 %), so tops, set lines and sigil lines align across the offer. The set block and the sigil are the card's foot group, anchored to the bottom edge. The who-wears-it columns are measured together the same way. The who-wears-it column carries the candidate's portrait chip AND the candidate themselves idling at `ACTOR_SCALE`, clipped to the column, in place of the word EMPTY, or four of `CARD_W_FOUR = 284` × 440 at x 48/348/648/948 under HASTE; the room-to-room card is the middle slot with `CONTINUE = (448, 552, 384, 96)`; the who-wears-it row is its own grid: `WEAR_BTN 280×96` at `WEAR_X = 40/344/648/952`, `WEAR_Y = 552` (the fourth is decline), each candidate's current piece and compare line in the three card slots |
| doors | DESCEND and ANOTHER LAP, 520×200 at `DOOR_X = 96 / 664`, `DOOR_Y = 320` |
| inspect | `INSPECT = (24, 24, 1232, 648)` via `drawPanel`: name at `TEXT_LABEL` (48, 40); then two columns. WHO — x 48, `INSPECT_COL_W = 356`: the element pictogram and word, current/max HP with its rule, the character themselves idling at `ACTOR_SCALE` (clipped to the column, centred between the HP rule and the numbers), and the eight derived stats the battle is resolved with (`HP ATK DEF SPD CRIT CDMG ACC RES`, two to a row). WHAT — from x 432: the six rows at `INSPECT_ROW_Y = 96 + 72 × i`, each on its own gradient plate — a 32-px slot **pictogram** (`SLOT_ICON_NAME`, amber when the slot is filled and dim when it is empty), the title, the main line under it, and all FOUR substats across the row's right half; an EMPTY row names its slot, says "nothing worn" and prints the mains that slot can roll. The foot band carries set bonuses in `SET_BAND = (48, 536, 968, 112)` at `SET_LINE_Y = 540 / 576 / 612` (or "no set is complete yet") and, from `SET_BAND.x + 512`, the character's three SKILLS with their cooldown and target. The overlay is `dimScene(0.62)` with the panel over it at plate alpha **0.92**, drawn with the battle HUD (panels, ribbon, log, command list, and the stage plane's own hairlines and head pictograms) suppressed beneath it, so no panel, number or log line ghosts through; `BACK` is the secondary button (text plus a focus underline), also bound to B |
...
| party | three member columns in the card slots, each with a head at y 56 (portrait, name, element, HP bar, a star on the leader, the AWAKENED mark) and six `PARTY_ROW = 64` slot rows from y 128 in `SLOTS` order as its `index`; a row registers its DRAWN 384 × 64 rect. **A row's action here is "select this column"** — empty rows included — because that is what the two conditional buttons act on; relics are never moved on this screen (the who-wears-it row at a card screen is the only mover). Set bonuses in `PARTY_SET_BAND = (640, 512, 304, 136)`, with its own label row at y 512. `PARTY_SWAP = (40, 552, 280, 96)`, `PARTY_LEADER = (344, 552, 280, 96)` (acts on the focused column; `disabled` outside the draft, a SUMMON, a REST and the ALTAR — i.e. precisely when a `leader()` decision is open) and `PARTY_BACK = (952, 552, 280, 96)`, group `party` — the six slot rows carry index 0–5, `PARTY_SWAP` index 8, `PARTY_LEADER` index 9, `PARTY_BACK` index 10 — BACK also bound to B. With a leader decision open BACK reads "KEEP <name>" and answers with the seat as it stands, which is that decision's own default. A face that reuses the columns registers them under its own prefix and group (`forge` / `altar` / `rest` / `bank` / `swap`) with the same geometry |
...
| shrine | the pact as one card in the middle card slot — `(448, 88, 384, 440)`, group `cards`, index 0 (an exception to the `cards` row above: the pact card registers its drawn, height-fitted plate, grown to `TAP_MIN` by the registry; the 440 band is not a target — e.g. `(448, 193, 384, 230)` for a one-line blurb) — name ≤ `NAME_MAX_PACT`, a CURSE line, a BOON line and the blurb ≤ `BLURB_MAX` wrapped to ≤ `BLURB_LINES_MAX` lines. Tapping the card accepts; `SKIP` (index 1) and B walk past, which is the default. Pacts already taken ride as chips from `(40, 128, 300 × 44, gap 8)`. No mend either way |
...
| vault | the twelve `DRAFT_CARD` slots are the Vault grid, group `vault`, index = slot. EQUIP: a chip is `disabled` once its `Slot` is spoken for or `vaultSlots` picks are made, `CONTINUE` (index 19) reads BEGIN THE RUN, and the ascension stepper is ONE control on the foot row left of it — `ASC_DOWN = (40, 552, 96, 96)` (index 20) · `ASC_PLATE = (144, 568, 200, 64)`, the level and its reason · `ASC_UP = (352, 552, 96, 96)` (index 21) — each arrow `disabled` at its own limit (the floor is `minAscensionFor(equipped)`, the ceiling `max(floor, unlocked)`); B clears the picks. BANK: the worn relics are picked on the party columns (group `bank`, at most `n`), `CONTINUE` (index 8) commits, and **only** when `vault + take` overflows `VAULT_SIZE` does a second face open on the same grid to pick the drops — `CONTINUE` there is never `disabled` on the drop count (the seam trims whatever is still over the cap, lowest level first, so an under-filled pick is completed for the player, not blocked) — with B stepping back |
