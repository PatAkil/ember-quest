...
**Status: revision 4, under review (2026-09-12).** This folder is the plan for building
...
agents in three rounds (112, 97 and 73 findings; round 3 returned one blocking finding and
27 gaps, all resolved here); `REVIEW.md` is the log of every round, and this README will
say so when a round closes with no blocking or gap finding. Until then, treat it as a
draft.
...
| D15 | Branch model and releases | `main` protected by a **ruleset** — a pull request required, required approvals 0 with a code-owner review on owned paths, stale approvals dismissed on push, required checks with always-run shim jobs, branches up to date, linear history, an empty bypass list; a **tag ruleset** on `v*` and `oracle-*`; the signing secrets only in a **`release` environment** the owner approves; **no merge queue** (unavailable on a user-owned repository) (§ T4.1, § V5) | a long-lived branch; an organization transfer for a queue | protection is per branch, so it covers the frozen prototype too; a transfer would move the live URL | one phase | **yes** (question 7) |
| D16 | Backdrops | Flat, lit frames captured once from the prototype as placeholders, drawn as one plane under the light data; the scene phase makes four planes per biome as data-driven painters or AI-generated planes (§ T9.2, § T10.9) | port the prototype's painters; no placeholders | the stage exists from P5 with nothing to bake; the scene is designed for the new cast, not inherited | one phase | no |
...
| P2 | reviews the golden set's commits (two or three pull requests); no clause reviews — the fold writes proposals |
...
| P1 The rig | The environment recipe proven on the agents' machine; the App, the ruleset, `CODEOWNERS`, the tag ruleset, the release environment, the private runner repository; every module wired; every lane with its budget measured on a synthetic `:core`-sized module and a synthetic storyboard; the analyser set; the spec binder; hooks and skills including the `kmp-quality` rubric | Every lane green and inside budget on the synthetic module, timings in the ledger | M |
