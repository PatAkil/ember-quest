...
**Status: revision 14, under review (2026-09-12).** This folder is the plan for building
...
agents in twelve rounds (112, 97, 73, 71, 55, 59, 69, 62, 59, 72, 73 and 56 findings;
...
is revision 14, after round 12, whose two blocking findings were one slip of the
consistency pass's own fix (a generator rule that failed on the root files) and one fact
about the prototype's harness (it exits non-zero on a cell that stalls, which the stall
fixture is meant to do). A read-only consistency pass over the four documents precedes
...
6. **What it costs and what stands still** (below): about 14–28 sessions of agent
   ...
   dependency batch; the demo stands
   still with its known defects. **Approving the plan
...
`FUNCTIONAL.md` § F4.3, § F5); without the scene phase 17–34; the web stretch adds 1–2. The critical path
...
about an hour a month over the programme. P1 alone takes about a week and a half; a sitting
...
| P5 | reviews the goldens and the `screens` and `platform` clause promotions, batched into one pull request per session (four to eight); walks the felt rows on a device; uses the ids and the App Store name fixed at P0; sets up the store listings, the privacy-policy page (`docs/privacy/index.html`, published beside the demo by the Pages workflow, `TECHNICAL.md` § T12), the data-safety form and the content rating; names the testers' channel in the listing and reads it weekly with the agents' triage, alongside the stores' own crash reports; invites the iPhone-holding testers to external TestFlight so iOS is tested by people other than the owner; loads P6's plane key ($500) only if question 4(b) approved it; creates the first `v*` tag and approves each `release`-environment deployment (the owner is its required reviewer, so every signed build waits on a click); adds the phase's new required checks to the ruleset when the L3 list check names them; **creates the Apple certificates repository, the App Store Connect API key, the Play upload key and the Play Developer API service account and stores them in the `release` environment** (no agent can); answers question 4. An absent owner blocks the first test-track build |
...
| The owner's review queue is the binding calendar term (fifty-seven to sixty-six sittings) | Batching into milestones; one pull request per owner-gated path with nothing stacked behind it; the absence protocol; § V7's review-queue measure | A pull request waiting on the owner more than five weekdays |
...
5. **The device lane (D18)** — closes at P1. A self-hosted runner on the owner's machine in
   ...
   macOS guest cannot take one — holding nothing but the phone and its token, and the
...
8. **The repository (D2)** — closes at P0's start, before the move commit, two parts. (a) *Where*: the prototype under
   ...
   *Visibility*: public, as today (recommended: the hosted runners, the arm64 leg, Pages
   ...
   too (≈ 2 000 free Linux minutes a month, then about $0.006 a minute; the nightly alone is
   ≈ 5 000 minutes a month), the free arm64 leg lost, and rulesets and Pages on a private
   personal repository only on GitHub Pro, so D15's whole enforcement model is behind the
   paid plan there — for a source nobody could otherwise copy? The root `LICENSE` is the
