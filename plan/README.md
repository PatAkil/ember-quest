...
**Status: revision 17, under review (2026-09-12).** This folder is the plan for building
...
agents in fifteen rounds (112, 97, 73, 71, 55, 59, 69, 62, 59, 72, 73, 56, 52, 51 and 48 findings;
every blocking, gap and numbers finding of each round was resolved in the next revision,
every minor one applied or listed in `REVIEW.md` with the reason it was declined); this
is revision 17, after round 15 — the second round with no blocking finding; its sixteen
gaps were the seat spread's population, the rig's unexported constants, the LOW bar's
venue, the hand-written workflows' check names, the cell line's slug, the Vault file's
root, a device lane that cannot assemble on an arm64 guest, a provider that cannot cap,
the store identity's refresh, a Play account that may predate the rule and a ruler with no
rework bound — every one is resolved here. A read-only consistency pass over the four documents precedes
...
`FUNCTIONAL.md` § F4.3, § F5); without the scene phase 17–34 of capacity; the web stretch adds 1–2 and question 2's portrait branch 4.5–6.5, the calendar moving with either; outside every band sit the contingent items: the felt-row worst case, up to 2–4 sessions at P5 (`FUNCTIONAL.md` § F1.5); P7's blockers beyond the first, an S each (the P7 row); question 3(c)'s ≈ 1 session, its recommended branch; the demo-unusable fix, ≈ ½; a second bake-off, ½–1; and option B's ≈ 8–12 — while question 1's painted branch adds its ≈ 1 session inside P4's L and its atlas-residency change — ≈ 50 MB of atlases against 12–14, one biome resident the lever — inside P5's XL. The critical path
...
and writing the character brief are the owner's own time, unsized here. P1 alone takes about a week and a half; a sitting
...
weeks that follow the build rather than overlap the programme, plus whatever the owner's
availability window adds (the absence protocol); the testers themselves are recruited
from P3's gate, a month before the build. A wrapper of the frozen prototype uploaded to
...
which needs three or more sessions a week, so a higher tier or a second seat, whose
...
running on `main`; the device lane reports SKIPPED when the phones travel with the owner (the
runner machine stays on); the review queue is the first item on
...
trigger, the second exception (the subscription is not renewed while a decision is
...
— the nine-to-ten-week floor is one sitting a weekday — and during P6.
...
is maintained. Until the first P5 build, the only thing anyone can play is that demo — an
...
6. **Option B reversed (D9)** — closes at P0's start, before the bake-off's spend. Confirm that AI-generated sprites replace the
   ...
   or the whole-cast re-gate mid-cast, a second P4 at 2–4 sessions and ≈ $1 500–2 500,
...
10. **Resume anywhere (D6, `FUNCTIONAL.md` § F2.1)** — closes at P0, because P3 designs
   ...
   (an S inside the M) with its corpus; a "no" saves about those 1–2 sessions of P5, the
