...
**Status: revision 12, under review (2026-09-12).** This folder is the plan for building
...
agents in ten rounds (112, 97, 73, 71, 54, 59, 69, 62, 59 and 72 findings; every
...
revision 12, after round 10, whose two blocking findings were in the plumbing — the
agents' App lacked the `actions` permission the golden-recording job needs, and a recipe
edit could never pass `env-check` — and whose gaps showed the previous revision's
mechanisms had grown baroque, so this one also simplifies: the art spend's hard ceiling is
the providers' own caps, a subscription winner is paid in months, and the in-scene bar is
measured on one fixed set of frames.
...
6. **What it costs and what stands still** (below): about 14–28 sessions of agent
   ...
   phase included) — on a calendar whose floor is the owner's forty-one to forty-seven
   review sittings up to the first test-track build (fifty-two to sixty over the
   programme) plus the stores' four weeks after it, about **twelve to fourteen weeks at
   three or more sessions a week**, **twelve to nineteen at two a week and nineteen to
   thirty-eight at one** — the one to two a week this review measured on the owner's
   current subscription; the owner's own time is about 45–90 hours of review sittings,
   15–25 hours of admin, store and recruitment work and about five hours of play; the demo
   stands still with its known defects. **Approving the plan
...
term binds instead: about twelve to nineteen weeks at two a week (the review floor still
binds at its low end), nineteen to thirty-eight at one. A fourth
...
| Google Play developer account | ≈ $25, plus identity verification (a government ID, a phone and a payment method) before anything ships; a developer name and a contact email shown on the listing (an address only under a trader declaration, as for Apple) | a personal account created after November 2023 must run a closed test with ≥ 12 opted-in testers for 14 continuous days per app before production access, the count may not dip, and Google's review checks that they used the app — the owner lines up **twenty to twenty-five** people with a platform mix — at least twelve on Android for Play and at least three on iPhone for external TestFlight, a tester with one phone serving one store — who will actually play, recruited when P3's gate goes green (about a month before P5's first build); the test starts on the first P5 build that passes the merge lane **and** falls in a window the owner can cover (the absence protocol), which may add weeks to the stores' term; if twelve Android testers cannot be held, the named fallback is to ship iOS first and leave Android in closed testing until the count holds, or a paid testing service priced at P5 |
...
| A store rejection, or a closed test that resets (App Review rejects the build; production access is refused; the opted-in count dips below twelve, which restarts the fourteen days) | The P5 store set-up with the owner's checklist and the privacy manifest entries; twenty to twenty-five testers with a platform mix for a floor of twelve on Android; the testers' channel read weekly; the stores' own crash reports; the fallback if the twelve cannot be held — iOS first with Android left in closed testing, or a paid testing service priced at P5; a rejection costs a second review cycle (days) and a reset closed test fourteen more days | Opted-in testers below fourteen; any rejection notice |
...
2. **Orientation** — closes at P0's start, after the baseline play (P0's first act, so the
   ...
   ruler (about half a session more): about five to six and a half extra sessions in all.
...
8. **The repository (D2)** — closes at P0's start, before the move commit, two parts. (a) *Where*: the prototype under
   ...
   *Visibility*: public, as today (recommended: the hosted runners, the arm64 leg, Pages
   ...
   paid plan there — for a source nobody could otherwise copy?
