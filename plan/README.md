...
**Status: revision 18, under review (2026-09-12).** This folder is the plan for building
...
agents in sixteen rounds (112, 97, 73, 71, 55, 59, 69, 62, 59, 72, 73, 56, 52, 51, 48 and 45 findings;
...
is revision 18, after round 16 — the third round with no blocking finding; its seventeen
gaps included the engine hook that lights a supplied plane, a golden re-recording with no
route, the blind verifier's missing API, the failures no module owns, a save two versions
old, the accepted cast's own in-scene reading, an empty field signal, the EU trader
declaration, the re-run rule left to discipline, the testers' phones against the minSdk
fork and the seat list written after the captures that plant it — every one is resolved
here. A read-only consistency pass over the four documents precedes
...
a session of agent capacity, and P0 may start with open minors; this README will say when a round
...
3. **The mechanics are the baseline; the presentation is new.** The prototype's rules are
   ...
   does not contain, and clause tests alone bind; the screens, the stage and the art are designed
...
6. **What it costs and what stands still** (below): about 14–28 sessions of agent
   ...
   plus the stores' four weeks after it, about **thirteen to sixteen weeks at three or
   more sessions a week**, **thirteen to nineteen at two a week and nineteen to
   thirty-eight at one** — the one to two a week this review's cap suggests (its weekly
   limit was hit twice inside a day of rounds; a round is not a session, so *n* is measured
   at P1, not here) on the owner's current subscription, the best case needing a second seat or a higher tier the money
...
10. **Enforcement is by machines where a machine can do it, and the plan says where it
    ...
    fallback captures and the sound hashes), the M2–M3 window at P1, whose owned-path pull requests land before the
...
phases need P0 ≈ 8 (the spike reports and the prototype commits about four; the art tool
and the captured assets, the calibration's adjudication, the critic's protocol and
baselines and the look decision about four), P1 ≈ 8 (about fifteen pull requests),
...
time, and it starts only at the first test-track build**: Play's review of the first
closed-testing release (days), the fourteen continuous days, Google's review of the
production-access application (up to about a week by Google's own statement, longer in
exceptional cases) and App Review (one to three days, longer on a rejection) — about four
...
one a day) and P7 not in fewer than about **thirteen to sixteen weeks** — the best case,
...
review's cap suggests** the first term binds instead: about thirteen to nineteen weeks at two a
...
|---|---|---|
| Agent compute | — | the owner's Claude subscription — the Max-class tier this review ran on, assumed at ≈ $100–200 a month for the arithmetic and recorded exactly here at P0, with whether the tier can rise and whether a second seat is permitted — the three-or-more-sessions branch exists only if one of them is — whose *n* sessions a week is the calendar's first term; this review hit its weekly limit twice inside a day of rounds, which suggests — a round is not a session, so it does not measure — about one to two sessions a week, measured at P1, and a higher tier or a second seat raises *n* at roughly the same again per month, which the best-case calendar assumes and the programme total prices as its three-or-more-sessions branch; a session of this size at API list prices would cost about ten times the month's subscription — the ratio is measured on one real session after P1 and recorded here, not guessed |
...
testers' invitations that passes the merge lane *and* falls in a window the owner can cover, the fourteen days and a week
...
| A generated actor resembles someone else's character or mark and a notice arrives — a takedown, a store removal (`FUNCTIONAL.md` § F3.6's residual) | criterion 8 and the prompt rule reduce it; the response pulls the asset, regenerates it against the accepted cast (≈ $15–30), tags, and passes App Review and a Play update again — an out-of-cycle release session | a notice from a store or a rights-holder |
...
1. **The look (D10)** — closes at P0's exit. Pixel sprites at the contract's cell under the
   ...
   critic's calibration sheet re-scored under its pinned, look-neutral prompt, the offset a
...
2. **Orientation** — closes at P0's start, after the baseline play (the owner's first act after the move commit, so the
   ...
   be re-captured — with the in-scene bars re-recorded on them at P5 and the bands
   re-derived at P0; plus the in-scene ruler re-derived
...
6. **Option B reversed (D9)** — closes at P0's start, before the bake-off's spend. Confirm that AI-generated sprites replace the
   ...
   knowingly; under a "no" the aggregate is 17–34 without P4's L, and option B's ≈ 8–12
