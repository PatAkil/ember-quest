...
**Status: revision 15, under review (2026-09-12).** This folder is the plan for building
...
agents in thirteen rounds (112, 97, 73, 71, 55, 59, 69, 62, 59, 72, 73, 56 and 52 findings;
...
is revision 15, after round 13 — the first round with no blocking finding; its twenty-one
gaps sat on the plan's edges (the tag ruleset's bypass list, the Gradle wrapper as a
second unowned pin, the recording workflow's check names, the forcing hooks' firing
points, the tier the testers actually see, the steady state's upgrade debt) and every one
is resolved here. A read-only consistency pass over the four documents precedes
...
3. **The mechanics are the baseline; the presentation is new.** The prototype's rules are
   ...
   (forfeit, `minAscensionFor`'s vector, `resumeRun`), which the oracle does not contain and
   clause tests alone bind; the screens, the stage and the art are designed
...
6. **What it costs and what stands still** (below): about 14–28 sessions of agent
   ...
   phase included; 17–34 under question 4(a)'s recommended branch, which runs the scene
...
`FUNCTIONAL.md` § F4.3, § F5); without the scene phase 17–34; the web stretch adds 1–2 and question 2's portrait branch 4.5–6.5, the calendar moving with either. The critical path
...
a WebView wrapper a rejection risk, and the testers would judge the art the owner
rejected. So at one sitting a weekday the first test-track build cannot arrive in fewer
...
which needs three or more sessions a week; at the **one to two sessions a week this
...
|---|---|---|
| Agent compute | — | the owner's Claude subscription — the Max-class tier this review ran on, assumed at ≈ $100–200 a month for the arithmetic and recorded exactly here at P0 — whose *n* sessions a week is the calendar's first term; this review hit its weekly limit twice, which is the measured case of about one to two sessions a week, and a higher tier or a second seat raises *n* at roughly the same again per month, which the best-case calendar assumes and the programme total prices as its three-or-more-sessions branch; a session of this size at API list prices would cost about ten times the month's subscription — the ratio is measured on one real session after P1 and recorded here, not guessed |
...
8. **The repository (D2)** — closes at P0's start, before the move commit, two parts. (a) *Where*: the prototype under
   ...
   *Visibility*: public, as today (recommended: the hosted runners, the arm64 leg, Pages
   ...
   too (3 000 included Linux minutes a month on GitHub Pro, then $0.006 a minute — the
   nightly alone is ≈ 5 000 minutes a month, ≈ $12 past the allowance — plus Pro's ≈ $4 a
   month), the free arm64 leg lost, rulesets and Pages on a private personal repository
...
9. **The agents' environment** — closes at P1, after P0's spike. Two independent reasons
   ...
   half a session to set up and an hour a month of upkeep beside its money — or the owner
   wants
