...
**Status: revision 13, under review (2026-09-12).** This folder is the plan for building
...
agents in eleven rounds (112, 97, 73, 71, 54, 59, 69, 62, 59, 72 and 74 findings; every
blocking, gap and numbers finding of each round was resolved in the next revision, every
minor one applied or listed in `REVIEW.md` with the reason it was declined); this is
revision 13, after round 11, whose one blocking finding was a gloss the previous revision
wrote on the red-`main` rule with its direction inverted. From this revision on, a
read-only consistency pass over the four documents precedes every round, because about a
third of each round's findings were residue of the previous revision's own edits.
...
spikes an L), P1 L (§ V9's eight milestones and about fifteen pull requests), P2 L, P3 L, P4 L
...
first-ten-minutes test about five more. P1 alone takes about a week and a half; a sitting
...
| P5 | reviews the goldens and the `screens` and `platform` clause promotions, batched into one pull request per session (four to eight); walks the felt rows on a device; chooses the bundle and application ids (permanent after the first upload; the name was reserved at P0); sets up the store listings, the privacy-policy page (`docs/privacy/index.html`, published beside the demo by the Pages workflow, `TECHNICAL.md` § T12), the data-safety form and the content rating; names the testers' channel in the listing and reads it weekly with the agents' triage, alongside the stores' own crash reports; invites the iPhone-holding testers to external TestFlight so iOS is tested by people other than the owner; loads P6's plane key ($500) only if question 4(b) approved it; creates the first `v*` tag and approves each `release`-environment deployment (the owner is its required reviewer, so every signed build waits on a click); adds the phase's new required checks to the ruleset when the L3 list check names them; **creates the Apple certificates repository and the Play upload key and stores both in the `release` environment** (no agent can); answers question 4. An absent owner blocks the first test-track build |
...
programme, where an absence over P4's sheet queue burns a paid month and can itself fire
the stop trigger, the second exception. Otherwise: the ruleset has no up-to-date
...
| Steady state (after P7) | Keeping the two listings alive: Google Play's annual target-API bump and Apple's SDK minimum for submissions, the stores' policy re-attestations, Renovate's monthly batch through the full lanes, a store-side fix when one is needed; the first session answers the closed test's rule-touching reports the owner chose | The lanes, unchanged; a `v*` tag per release | ≈ one session a quarter, which covers the annual target-SDK bump |
...
2. **Orientation** — closes at P0's start, after the baseline play (the owner's first act of P0, so the
...
4. **The scene phase** — two parts, both closing at P5's end. (a) *Ship before or after
   ...
   composition axes at least one above P0's baseline, capped at 9)? The closed testers see
