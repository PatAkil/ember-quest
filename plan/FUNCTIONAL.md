...
| Quality tier defaults | HIGH everywhere (the auto-drop to LOW and the ARCADE toggle) | by a first-launch benchmark — MED on a 2022 phone, HIGH where the stage holds 60 Hz once P6's planes land with HIGH's bar (until then MED is the picker's top, since P5's flat stage has no HIGH bar, `TECHNICAL.md` § T9.2) — adjustable in settings; the auto-drop stays | a 2022 phone cannot hold HIGH at 60 Hz |
...
| F2.5 | **App identity** | the app icon (a 1024 × 1024 master — EMBER's accepted idle frame 0 on its 64-cell canvas at 16× — 64 × 16 = 1024 exactly, the silhouette centred on the canvas, nearest-neighbour (under question 1's painted look, the 128-px frame at 8×, bilinear), the fallback hero's until the cast is accepted and re-cut from the accepted hero before the P7 tag — or, where the heroes' acceptance is deferred to P6 under question 4(a)'s recommended branch, in P6's release line with the screenshots — with the store screenshots re-shot over the accepted cast (and the screenshots and the feature graphic re-shot again over P6's scene under question 4(a)'s recommended branch, in P6's release line) — a listing update on both stores, an S, and one owner sitting on the P7 row — over the ground colour recorded at P0 in `spec/art/seats.md` (`spec/fixtures/art/seats/seats.json`'s `ground`, `TECHNICAL.md` § T7.5), produced by `frames --icon` (`VERIFICATION.md` § V3) and never generated — from which the platforms' sizes and Android's adaptive layers are derived by the build), the splash (the icon on the ground colour through each platform's own launch screen), Play's 1024 × 500 feature graphic (the same crop over a stage capture); all three under `assets/store/`, an owned path the owner reviews on the P5 row; store listing, a credits screen that names the AI art providers and every bundled asset's licence (the HUD face's OFL or Apache notice; the sounds and the glyph tables are the prototype's own) | Store metadata is copy the owner writes; the credits line is required by § F3.6. | S | P5 (identity and the listing copy, which Play needs before a closed-track release); P7 (the credits and disclosure copy) |
...
- **Value.** The figure sits dark on a lit ground. *Pass* (the sheet criteria the prototype
  ...
  **retired** because the shadow band the bar needs — L 35–48 — cannot clear 3:1 against
  ...
  `flat=1 tier=LOW` path over the committed composite (`TECHNICAL.md` § T4.2), not on
  `bakeFlat`'s own frame, whose floor is resampled — and at the tier the reference
  ...
  diorama, which is not the scene P5 draws; and P6 at HIGH on the landed rig — the tier is not offered before P6 (§ F2.6) — against
  P0's diorama frames captured with the bloom off as well (both HIGH sets pass `bloom: false`,
  `TECHNICAL.md` § T4.2), with LOW and MED re-read over the landed planes and not below their recorded bars (P0's, or
...
it inline in the pull request body — the lit crypt frame is a panel of that sheet, not a
file of its own — since the candidates are never
...
before the first verdict of spike 7 or spike 8, whichever runs first, and promoted unchanged into `spec/art/` at P0's
...
frozen calibration sheet — the prototype's round-14 cast in lit frames with the HUD, a hit
peak and one resting stage frame per biome, the full-frame set the five axes are scored on, committed as
...
2. **After the six heroes (in P4):** the party on the prototype's stage in three biomes,
   ...
   same rule, each with a no-regression rule on the axes an earlier phase has already read (the sprite axis at P5; sprite, UI and VFX at P6) (`TECHNICAL.md` § T14); an axis
   ...
   *Continue* to the enemies; *change provider* (the whole-cast re-gate, README money
   ...
   the cast.
3. **When a key's cap is reached** — the counter's refusal at 90 % of that key's own cap
   ($3 150 on the sprite key, $450 on the portrait key; the $4 000 ceiling is the two caps
   together) or, for a subscription winner, a month that ends with the cast
   ...
   (a subscription winner's allowance paces P4 over months, README); where the heroes are
   late the owner's acceptance of those actors is taken at P6, the felt rows having been
   walked over the fallback heroes.
...
(`TECHNICAL.md` § T10.2 names the current candidates and their terms, verified at P0, re-read at P4's start and before any later character's generation, and
