...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  written by `setup.sh` at session start, so an edit to the recipe is unverified until L3; L3, where a dedicated `env-check` job runs `setup.sh` on the **bare**
...
- **M3 — the rig's skeleton.** The convention plugins and the empty modules with the edge
  ...
  the ledger.
- **M4 — the gates.** The required checks added to the ruleset (every lane job, every
  `changes` job, the generated-workflows job, the three owner-review checks from the gate
  App), the tag ruleset and the `release` environment; Renovate installed → **the second
...
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  semantics test with virtual time → one Roborazzi golden recorded in the image and
  compared on the owner's Mac → the storyboard driver crossing one screen in both modes →
...
P1 ends when every lane is green *and* every P1-measurable budget holds with the ledger to
show it, and the owner has accepted the re-derived sizes and calendar or stopped. If a
