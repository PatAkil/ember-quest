...
3. **First rules change after parity**: the goldens move with a clause, `RULES_VERSION`
   ...
   and is retired from the nightly, and `prototype-check` with it — the prototype workflow keeps the freeze check alone from then on, the harness's rot repair leaves § T4.2's allowed changes, and the Pages site keeps publishing `docs/**` through its own job whatever the prototype build does (§ T4.1); `prototype/` stays in the repository as history and as
