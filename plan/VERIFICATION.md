...
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   64-cell canvas at 16× (64 × 16 = 1024, the silhouette centred, § F2.5) over `seats.json`'s recorded `ground` (`TECHNICAL.md` § T7.5), and the feature graphic over a stage
   capture;
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  versions (the JDK, Gradle, Node, every Android SDK package `versions.env` names — the manifest's scope is what the recipe installs, at the versions found, under `setup.sh`'s own SDK root, never what a host already holds — the fonts, and a `marker`
...
P1 and monthly after, and past 500 MB the remedy is Git LFS for `spec/golden/images/**` and
`assets/**` within the public quota, or those two checkouts narrowed to the tag and the merge
base so history stops mattering; flakes (target
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → the
  ...
  missing report each fail a fixture lane) → Konsist and the JVM ABI dump → the verifier's stub generator (`:tools:stub`, § T6.3), an S inside P1's L → the `ART` clauses promoted to `contract` in the same pull request, bound by the art tool's gate tests and the calibration golden from P1 (§ T7.2's stateless rule would otherwise fail every pull request from here), under the owner's contract-clause review; the `BALANCE` clauses are P3's, promoted with the rules clauses once the reduced snapshot binds them.
