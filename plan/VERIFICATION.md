...
rules against the merge ref) and, like `pull_request_target`, runs `main`'s copy of the file (`GITHUB_REF` is the base branch), so the head branch cannot rewrite it either — and it is still
