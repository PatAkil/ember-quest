...
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   `--seats all` fills every anchor — how the ruler frames of P5's and P6's gates are made
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → the
  calibration golden's second reader, written from § T10.4's definitions without reading
