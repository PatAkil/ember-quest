...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  created in § V9's M3), which compares the freshly built image's manifest with the committed
...
- **M3 — the rig's skeleton.** The convention plugins and the empty modules with the edge
  ...
  the ledger, and `.github/workflows/env-image.yml` rebuilding the image and re-checking
  its manifest.
- **M4 — the gates.** The required checks added to the ruleset (every lane job and every
  ...
  generated-workflows job, the three owner-review checks from the gate App), the tag ruleset and the `release` environment; Renovate installed → **the second
