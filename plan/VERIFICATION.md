...
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  a hard failure, § V4 — and byte-exact in L3 on a different hosted-runner generation, a divergence naming § V4's fallback → the storyboard driver crossing one screen in both modes →
