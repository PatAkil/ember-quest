...
otherwise, naming the slowest task. Also tracked: repository size — the clone the freeze check's and `release.yml`'s
`fetch-depth: 0` pull on every pull request, where the image goldens, the contact sheets, the
