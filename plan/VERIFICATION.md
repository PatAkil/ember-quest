...
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   at 16× over the bible's recorded ground colour, and the feature graphic over a stage
   capture;
   `--cast fallback` plants the fallback cast whatever `assets/actors/**` holds and
   `--seats all` fills every anchor with the biome's six seat actors from `spec/fixtures/art/bible/seats.json`, the bible's
   per-biome seat list (`TECHNICAL.md` § T7.5), the population the prototype's `seat=all` plants, and `--seat <id>` plants one id at all six anchors for the seat-spread reading, as the prototype's `seat=<id>` — how the ruler frames of P5's and P6's gates are made
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → the
  ...
  `tools/art`, asserted equal to `spec/art/calibration.json` within § T10.4's tolerances (`TECHNICAL.md` § T10.4) → Kotest on
...
- **M7 — the hooks and the platforms.** The hooks (edit, pre-commit, pre-push) → the
  ...
  it from an APK the owner downloads by hand from a merge-lane run (`gh run download`; the token that automates it exists only once question 5 chooses the runner) — the guest assembles nothing, since it is
