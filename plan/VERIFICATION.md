...
- **Rendering**: goldens are recorded and compared **inside the image**. An agent whose
  ...
  **`record-goldens`** workflow — its own `workflow_dispatch`-only workflow with a
  ...
  through `record-goldens` and compares it in L3 on a different hosted-runner generation,
  and a divergence names the fallback, the SIMD tier pinned through Skia's build flags in
  the image or a per-CPU-class tolerance on the goldens — and outside the image (an agent session, a macOS or Windows
...
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  and compared on macOS (the owner's Mac, or a hosted-macOS job) and in L3 on a different hosted-runner generation (the SIMD question of § V4), where the step asserts
