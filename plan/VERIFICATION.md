...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  manifest or `lanes.yaml`'s digest differ from what it built and passes once the agent
  has committed them, so a recipe edit is never deadlocked on a manifest that could only
  exist after its own merge; on `main` it re-tags the image the pull request built.
  "Proven equivalent" at P1 means the check passing on `agent-env`, on the owner's Mac and
  in L3. The agents' cloud
...
- **Rendering**: goldens are recorded and compared **inside the image**. An agent whose
  ...
  **`record-goldens`** job (`workflow_dispatch`, one screen or all), which renders inside
...
the bootstrap needs: the owner creates `CODEOWNERS` and *then* the provisional ruleset
...
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  and compared on the owner's Mac → the storyboard driver crossing one screen in both modes →
