...
2. **`storyboard`** — drive a seeded run through the *real* screens, tapping by test tag or
   ...
   in-battle so a real DEATH plays, as the prototype's `ko=1` does — and the prototype has only that one, so the rest
...
7. **`art`** (`tools/art/`, TypeScript) — `art gate <dir> [--ground <hex>]` (the sheet, motion and
   ...
   rulers <frame.png>… [--anchors <json>] [--ground]` (one frame or several at once, each frame's anchors read from its own `<frame>.anchors.json` unless one `--anchors` is given for a single frame; the seat ruler, the two ground strips, the seat
   spread over any battle frame — the prototype's at P0, the Kotlin stage's from P5 — and with
   `--ground` the strips' per-channel sRGB median over every frame given — the six MED `flat=1` frames at once for `seats.json`'s `ground`), `art
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  the digest as an artifact the agent commits into the same pull request — with the `kmp-*.yml` regenerated from the new `lanes.yaml`, or the generated-workflows job fails that pull request — (the
