...
2. **`storyboard`** — drive a seeded run through the *real* screens, tapping by test tag or
   ...
   **playback mode** (the merge lane): every turn is played at its paced length in virtual
   time and one PNG is written per distinct screen crossed. The same driver — its tap-and-assert logic in `:ui`'s test source sets, `:tools:instruments` the JVM CLI over it (`TECHNICAL.md` § T2.1) — runs as
   `androidInstrumentedTest` and `iosSimulatorArm64Test` on Android and iOS nightly.
...
7. **`art`** (`tools/art/`, TypeScript) — `art gate <dir>` (the sheet, motion and
   ...
   rulers <frame.png>… --anchors <json> [--ground]` (one frame or several at once; the seat ruler, the two ground strips, the seat
...
10. **The review bundle** — `build/review/<sha>/`: the matrix, the test summary, the lane
    ...
    goldens side by side), since a linked artifact does not render there.
