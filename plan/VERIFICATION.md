...
Four things stay with discipline and are audited by the next machine gate: the edit hook (L0) reports and cannot block; the git hooks (L1, L2) can be
...
`{lane, sha, machine, seconds, result, failed step}` as `lanes.json` into the review bundle;
...
and the contract-clause check. All three run on `pull_request_review` events (`submitted`,
`dismissed`) as well as on pushes, so an approval given after a check ran turns it green
without a new push; and all three run under `pull_request_target`, from `main`'s copy of
the workflow, checking out the pull request's head only to compute its diff and never
executing anything from it — so a pull request cannot rewrite the check that gates it.
The owner never authors a change on an owned path **or a contract clause**, since an
author's own review never counts.
...
| The lanes themselves | the CI workflows are generated from `ci/lanes.yaml`; a check, run from `main`'s copy of the workflow under `pull_request_target`, fails when the committed workflows differ from the generated ones; and `.github/**` and `ci/**` are owned paths, so a change to them needs the owner's review on the head commit | a tool silently disabled; a gate rewritten by the pull request it gates |
| The device runner | the self-hosted runner is registered in a **separate private repository** that the App is not installed on; that repository's nightly workflow runs on `schedule`, checks out `main` of this one, posts its results back as a commit status with a token it alone holds, and appends its `lanes.json` rows to the nightly's issue so `device-runner` rows reach the ledger; no workflow of this repository can name the runner | an agent's or a fork's pull request reaching the owner's machine |
...
| The environment's two forms drift | `gate.sh env-check` over the version manifest | L0 · L3 |
...
phones). **The ledger** is the union of every bundle's `lanes.json` uploaded as a CI
artifact and the `agent-env` rows carried in pull request bodies (`TECHNICAL.md` § T13.5);
every row carries its machine class. The nightly lane asserts each lane's p90 duration
