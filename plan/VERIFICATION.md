...
people and critics; the critic's score is a review under a protocol, never a CI gate — it is a phase-exit criterion at P4, P5 and P6 (§ T14); and the M2–M3 window at P1, whose owned-path pull requests (the ruleset, the workflows, the gate App's wiring) land before the owned-path check exists at M4, gated only by `CODEOWNERS` and audited by M4's second throwaway pull request. Where
...
2. **`storyboard`** — drive a seeded run through the *real* screens, tapping by test tag or
   ...
   contingent; a forced run carries its forcing hooks as a `debug:` preamble that `sim
   replay` applies before the first decision, and such a save is not portable across
...
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   the anchors file of `TECHNICAL.md` § T10.4 beside it so the art tool's rulers can read it.
...
6. **`sim`** (`:sim`) — `sim <command> [flags]`: `battles`, `runs`, `selfcheck`, `spd-gate`,
   ...
   battles knobs `--party --pack --act --lap --clears --pacts` with the prototype's defaults.
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  created in § V9's M3), which runs on `pull_request` when `ci/env/**` changed (its own
  `changes` job; a required check named `env-image`), on `workflow_dispatch` and on `push`
  ...
  registry and rebuilds only when `ci/env/**` changed since that commit, so the hand-back
  commit itself triggers no rebuild and the check verifies the pinned digest and the
  ...
  rebuilds from `main`'s recipe and turns `main` red (the red-`main` rule) if the rebuilt
  manifest differs from the committed one — the recipe pins its base image and package
...
- **Rendering**: goldens are recorded and compared **inside the image**. An agent whose
  host cannot run the image records a golden through the merge workflow's
  **`record-goldens`** job (the merge workflow carries a `workflow_dispatch` trigger with a
  `screens` input, on which the job runs and `changes` outputs all-true; one screen or
  all), which renders inside
...
**or a contract clause**, since an author's own review never counts — with two exceptions, the logged
ruleset edit of the infrastructure-failures row below and the one the bootstrap needs:
...
| The lanes themselves | the CI workflows are generated from `ci/lanes.yaml`; the **generated-workflows job** — an ordinary `pull_request` job of the merge workflow, head code, no secrets, a required check like any lane job — runs the head's generator and fails when the committed `kmp-*.yml` differ from what it generates (`prototype.yml`, `pages.yml`, `env-image.yml` and the two owner-review workflows are hand-written and outside the diff); it cannot run under `pull_request_target`, since that would execute the head's generator with the privileged token, and it need not: its own defeat needs a change to `.github/**`, `ci/**` or `build-logic/**`, which are owned paths, so the owner's review on the head commit is the gate behind it | a tool silently disabled; a gate rewritten by the pull request it gates |
| The device runner | the self-hosted runner is registered in a **separate private repository** that the App is not installed on; that repository's nightly workflow runs on `schedule`, checks out `main` of this one, posts its results back as a commit status with a token it alone holds, and appends its `lanes.json` rows to the nightly's issue — the ledger's third source; no workflow of this repository can name the runner | an agent's or a fork's pull request reaching the owner's machine |
| Infrastructure failures | an emulator or simulator that fails to boot, a runner evicted, a network error before any test body ran: re-run **once**, the re-run and its reason recorded as a comment on the pull request (or in the nightly's issue), which § V7's count reads; a second failure is real. A red required check can be overridden only by the owner editing the ruleset, and the edit is a logged comment on the pull request naming the check and the reason | a flaky test disguised as infrastructure; an unlogged override |
...
device-runner repository's rows appended to the nightly's issue (`TECHNICAL.md` § T13.5;
...
- **M2 — the review mechanics.** The owner creates `CODEOWNERS` and *then* the
  **provisional ruleset** (a pull request required, approvals 0, a code-owner review
  ...
  proves whether a code-owner review is enforced at zero required approvals.
- **M3 — the rig's skeleton.** The convention plugins and the empty modules with the edge
  ...
  workflow, and the owner-review workflow with its review-event re-dispatch (§ V5;
  ...
  the ledger, and `.github/workflows/env-image.yml` building the image on each `ci/env/**` pull
  request and handing back the manifest and digest to commit.
- **M4 — the gates.** The required checks added to the ruleset (every lane job and every
  ...
  generated-workflows job, the three owner-review checks from the gate App — the list `ci/required-checks.txt` the generator writes, which an L3 job checks against the branch's active rules from then on), the tag ruleset and the `release` environment; Renovate installed → **the second
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → Kotest on
  ...
  missing report each fail a fixture lane) → Konsist and the JVM ABI dump.
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  and compared on macOS (the owner's Mac, or a hosted-macOS job) → the storyboard driver crossing one screen in both modes →
...
- **M8 — the budgets.** **The synthetic `:core`-sized module** (generated into `:core`
  ...
  histogram: 5 000 lines, 300 tests including property tests; both deleted in P3's first
...
`env-check` and the size test over the hello-world build; for L4 the nightly skeleton's
