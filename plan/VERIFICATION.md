...
| **L1 module** | ≤ 90 s (`agent-env`) | the pre-commit hook; by the agent after each milestone; `gate.sh module` | the touched modules' `jvmTest` (`Fast` tag: unit, property at 100 cases, table-driven, screen semantics with virtual time); Konsist scoped to the touched modules; the JVM ABI dump check; the spec matrix for the clauses whose `paths` intersect the touched modules, less the `ART` and `RUN-FLOW` clauses, whose tests are L2b's (`TECHNICAL.md` § T7.2); the art tool's own tests when it changed | the rules and screens do what their clauses say; the architecture holds |
