...
people and critics; the critic's score is a review under a protocol, never a CI gate — it is a phase-exit criterion at P4, P5 and P6 (§ T14); and the M2–M3 window at P1, whose owned-path pull requests (the ruleset, the workflows, the gate App's wiring) land before the owned-path check exists at M4, gated only by `CODEOWNERS` and audited by M4's second throwaway pull request; and the art programme's commits before M4 — P4's generation starts at P0's exit, so an actor can be gated and accepted while neither the owned-path check nor L2b's asset gate exists — whose pull requests stay open until M4 merges them, with the asset gate re-run over every actor accepted before M4 at P1's exit (§ V9). Where
...
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   `frames --icon` writes § F2.5's icon master, splash and feature graphic from an accepted
   hero's idle frame at 16× over the bible's recorded ground colour;
   `--cast fallback` plants the fallback cast whatever `assets/actors/**` holds and
   `--seats all` fills every anchor with the biome's six seat actors from `spec/art/bible.md`'s
   per-biome seat list, the population the prototype's `seat=all` plants, and `--seat <id>` plants one id at all six anchors for the seat-spread reading, as the prototype's `seat=<id>` — how the ruler frames of P5's and P6's gates are made
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  created in § V9's M3), which runs on `pull_request` when the recipe changed — `ci/env/env.Dockerfile`, `setup.sh`
  or `versions.env`, never `manifest.image.json`, which is the hand-back's own artifact
  (its own `changes` job on those three globs; a required check named `env-image`), on `workflow_dispatch` and on `push`
...
device-runner repository's rows appended as comments to a standing `ledger` issue the owner creates at M7 — never the failure issue, which exists only on a red night (`TECHNICAL.md` § T13.5;
...
3. **First rules change after parity**: the goldens move with a clause, `RULES_VERSION`
   ...
   and is retired from the nightly, and `prototype-check` with it — the prototype workflow keeps the freeze check alone from then on, and the harness's rot repair leaves § T4.2's allowed changes; `prototype/` stays in the repository as history and as
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → the
  ...
  `tools/art`, asserted equal to `spec/art/calibration.json` (`TECHNICAL.md` § T10.4) → Kotest on
...
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  a hard failure, § V4 → the storyboard driver crossing one screen in both modes →
...
- **M7 — the hooks and the platforms.** The hooks (edit, pre-commit, pre-push) → the
  ...
  USB from the Linux VM on the owner's Mac by the real `adb`, and one `connectedCheck` run on
  it from an APK downloaded from a merge-lane run — the guest assembles nothing, since it is
