...
2. **`storyboard`** — drive a seeded run through the *real* screens, tapping by test tag or
   ...
   contingent), which `sim replay` reproduces headlessly and a verdict line: `PLAYFULL OK`
...
6. **`sim`** (`:sim`) — `sim <command> [flags]`: `battles`, `runs`, `selfcheck`, `spd-gate`,
   ...
   (the home-act pack generator; the battles knobs `--party --pack --act --lap --clears
   --pacts` with the prototype's defaults), `coverage` (the trace-coverage report: every event kind,
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  request's first build pushed under the head tag — the workflow rebuilds only when
  `ci/env/**` changed since its last build on the branch, so the hand-back commit itself
  triggers no rebuild — and passes once the agent has committed them, so a recipe edit is
...
- **M7 — the hooks and the platforms.** The hooks (edit, pre-commit, pre-push) → the
  Android, iOS and wasm compiles in L3 → the nightly skeleton and the private
...
schedule, its issue on failure, the device runner's SKIPPED row and the ledger check; the
