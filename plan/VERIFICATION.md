...
- **Rendering**: goldens are recorded and compared **inside the image**. An agent whose
  ...
  **`record-goldens`** workflow — its own `workflow_dispatch`-only workflow with a
  ...
  takes a per-CPU-class tolerance on the goldens, the CPU feature set recorded beside each
  golden — the one executable fallback, since Skiko ships a prebuilt `libskiko` and Skia's
...
- **M2 — the review mechanics.** The owner sets the repository's Actions settings (the default token read-only; Actions may not create or approve pull requests, `TECHNICAL.md` § T1), creates `CODEOWNERS` and *then* the
