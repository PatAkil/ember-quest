...
CI uploads every bundle as a workflow artifact; the nightly aggregates the week's artifacts
(and the Actions API's job durations) per machine class and checks each lane's p90.
...
| The prototype | `prototype/**` is code-owned, and a **separate workflow, `.github/workflows/prototype.yml`**, runs on every pull request that touches `prototype/**` with **no shim job**: the **prototype-freeze check** (`git diff --quiet ts-oracle-v3 HEAD -- prototype/game/sim prototype/game/data prototype/game/types.ts prototype/game/screens/vault.ts`, against the tag once it exists) and **`prototype-check`** (`npm ci && npm run check && npm run build && npm run smoke && node sim/run.mjs --selfcheck --runs 20` inside `prototype/` on the pinned Node), both required checks from P1 — so the P0 captures and the P2 harness changes are merged under the prototype's own gates and the harness that records the goldens is proven runnable; the allowed changes are `TECHNICAL.md` § T4.2's list, each in a named pull request | drift in the reference; a broken oracle harness |
...
phones). Every lane run's `lanes.json`, in its review bundle and uploaded as a CI artifact,
carries its machine class. The nightly lane asserts each lane's p90 duration over the last
week's artifacts, per machine class, is inside its budget and files an issue otherwise,
naming the slowest task. Also tracked: flakes (target
