...
produces, the fallback captures, the sound hashes, and `spec/art/bible.md`, `spec/art/seats.md` and `spec/art/rulers-p0.json` — the bands, the seat list, the ground colour and the three bars every P4–P6 gate is measured against — land before any machine gate exists,
...
people and critics; the critic's score is a review under a protocol, never a CI gate — it is a phase-exit criterion at P4, P5 and P6 (§ T14); and the M1–M3 window at P1 — the environment recipe first, committed in place by spike 6 and reviewed at M1 before `CODEOWNERS` or any check exists — whose owned-path pull requests (the ruleset, the workflows, the gate App's wiring) land before the owned-path check exists at M4 (the required-check list job of M3's merge workflow passing while the branch has no required-checks rule, until M4's ruleset carries the list, § T13.5), gated only by `CODEOWNERS` (required approvals 1 from M2 to M4 if M2 finds code-owner review unenforced; M1's recipe lands before the ruleset, on discipline audited by M4) and audited by M4's second throwaway pull request; and the art programme's commits before M4 — P4's generation starts at P0's exit, so an actor can be gated and accepted while neither the owned-path check nor L2b's asset gate exists — whose pull requests stay open until M4 merges them, with the asset gate re-run over every actor accepted before M4 at P1's exit (§ V9). Where
...
| **L1 module** | ≤ 90 s (`agent-env`) | the pre-commit hook; by the agent after each milestone; `gate.sh module` | the touched modules' `jvmTest` (`Fast` tag: unit, property at 100 cases, table-driven, screen semantics with virtual time); Konsist scoped to the touched modules; the JVM ABI dump check; the spec matrix for the clauses whose `paths` intersect the touched modules and whose binding tests carry a tag L1 runs (`TECHNICAL.md` § T7.2's rule — today that leaves out the `ART`, `RUN-FLOW`, `GOLDEN` and `BALANCE` clauses, whose tests are the commit lane's); the art tool's own tests when it changed | the rules and screens do what their clauses say; the architecture holds |
| **L2a commit** (`commit-a`), measured at P1 on the synthetic module and re-measured at P3 against the recorded goldens and the reduced snapshot, which the synthetic module cannot carry | ≤ 5 min (`agent-env`; with L2b the pre-push gate is ≤ 10 min in all, the two lanes' rows summed) | the pre-push hook (`gate.sh commit`), which runs L2a and L2b | the whole JVM test set; the committed golden hash lists; the reduced balance snapshot; detekt with type resolution over the tree; Konsist over the tree; Kover thresholds; the desktop perf test on the stage (allocation-based and relative to a same-run baseline, § T9.5) | nothing observable changed unless a clause changed |
| **L2b commit** (`commit-b`), content-scaled | target ≤ 5 min; its storyboard step measured at P1 on the synthetic storyboard, its content-scaled steps first measured at P4 (the asset gate over the cast) and P5 (frames, goldens, the real storyboard); if it outgrows the target at P5 the storyboard moves to L3 and § V6's rows move with it, as a README decision | the pre-push hook | screenshot goldens — byte-exact inside the environment image, SKIPPED-GOLDEN with a tolerance report outside it, which the hook accepts (§ V4); the storyboard in **skip-playback mode** (every biome, boss and screen reached with the strong party and the forcing hooks, once by tag and once by the keyboard route; a two-act run to a KO); the `vfx` and `backdrops` goldens (every image golden lives under `spec/golden/images/<kind>/`, § T2.1); the asset gate over `assets/actors/**` (every criterion re-measured on the committed frames; the halo is the one accept-time reading, taken under the owner's review and read from each sidecar since the raw candidates are not committed — the recorded hash names the raw, it re-measures nothing, § T10.1); the spec matrix over every clause whose binding tests carry a tag the lane runs, bound from L2a's and L2b's reports together (a clause bound only by `Device`- or `Nightly`-tagged tests is L4's matrix, `TECHNICAL.md` § T7.2); the calibration golden (`spec/art/calibration.json`, `TECHNICAL.md` § T10.4) and its second reader; from P5, `art rulers` over each biome's resting ruler frame (`frames --cast fallback --seats all --bloom off`, § T10.4 — not the shipped golden, which keeps its bloom; reported outside the image and gated only in L3, as the image goldens are, § V4, since the Skia SIMD tier the image cannot pin can move a reading across the 1.5:1 boundary) — the fixed set of 36 seats and 72 strip readings with the fallback cast as the reference, at LOW and at the reference phone's default tier (MED) from P5, and at HIGH as well from P6 — each at the in-scene bar recorded at P0 for that tier, a count of the 72 readings at ≥ 1.5:1 — the gate test sums the six biome frames' twelve readings each and compares the total to the clause's bar (`FUNCTIONAL.md` § F3.1) —, the seat spread reported beside it, not gated (the bars re-recorded at P5 under question 2's portrait branch, and at the reached count when the owner accepts a ruler miss, § F3.1); the review bundle | the screens and the assets did not change unless approved |
| **L3 merge** | ≤ 30 min wall-clock across parallel jobs (`hosted-linux`; one `hosted-macos-arm64` job ≤ 20 min for the iOS simulator) | the pull request (required checks — the `changes` job of every workflow that runs lane jobs on a pull request among them, named after its workflow — whose heavy jobs skip themselves by path condition; a ruleset, no queue, no up-to-date rule) | L2a and L2b re-run in the environment image (the byte-exact goldens); every target compiles (Android, the iOS framework; wasmJs as an informative, non-required job until P8) and the app, androidTest and Macrobenchmark APKs are uploaded as artifacts for the device lane; the klib ABI dumps (on the macOS job, inside its budget); the cross-platform hash test on the JVM, on an x86-64 Android emulator with KVM, on the iOS simulator and — if P0's spike 5b passes — on the JVM of a free `ubuntu-24.04-arm` runner, the arm64 arithmetic truth in the merge lane (wasm informative); `diff-oracle` on the probe subset (Node pinned; gated on `core \|\| prototype \|\| sim` — the booleans its red-`main` status names, § V5 — with § T4.1's `build \|\| root \|\| spec \|\| assets`); the full-playback storyboard with frames; Android Lint; `buildHealth`; the size test; the emulator and simulator boot smoke with the audio-plays assertion; workflow and script lint; the generated-workflows job (head code, no secrets); `env-check` on the bare runner (§ V4); the gate App's three checks (`owner/owned-path`, the approval read on the head commit through GitHub's review data; `owner/goldens`; `owner/contract-clause`, § V5); the prototype workflow (the freeze check unconditional, `prototype-check` skipped by its condition when neither `prototype/**` nor, from P2, `spec/**` changed); the nightly status read from the last nightly run; the rules-change golden-diff check (§ V5) | it ships on every platform |
...
1. **`shot`** — render any screen state from a fixture to a PNG, headlessly, in process:
   `instruments shot --screen map --fixture act3 [--phone] [--tier LOW] [--store <class>]`, at `k = 1`,
   1280×720, with virtual time (`--store` letterboxes that frame at the canvas's own `k = h/720`, as the device draws it, into
   the store class's canvas, § T12); one thin wrapper over the desktop headless scene so a
...
2. **`storyboard`** — drive a seeded run through the *real* screens, tapping by test tag or
   ...
   a hero's hp — `--ko` is that last hook under its own name, `hp@<i>=<slot>:1` on every living slot at
   the decision before the next battle (the record repeats per slot), the KO then dealt
   in-battle so a real DEATH plays — a wipe by design, to reach GAME OVER; the prototype's `ko=1` sets one hero to 1 hp and its run survives — and the prototype has only that one, so the rest
   ...
   **playback mode** (the merge lane): every turn is played at its paced length in virtual
   time and one PNG is written per distinct screen crossed. The same driver — main code in the `:storyboard` module, `:tools:instruments` the JVM CLI over it (`TECHNICAL.md` § T2.1) — runs as
   that module's own `androidInstrumentedTest` and `iosSimulatorArm64Test` on Android and iOS nightly.
3. **`frames`** — battle frames at chosen virtual ticks per biome and tier: the resting
   ...
   `frames --icon` writes § F2.5's icon master and splash from EMBER's accepted idle frame 0 on its
   64-cell canvas at 16× (64 × 16 = 1024, the silhouette centred; the 128-px frame at 8×, bilinear, under look B, § F2.5) over `seats.json`'s recorded `ground` (`TECHNICAL.md` § T7.5), and the feature graphic over a stage
   capture;
   ...
   per-biome seat list (`TECHNICAL.md` § T7.5), the population the prototype's `seat=all` plants, and `--seat <id>` plants one id at all six anchors for the seat-spread reading, as the prototype's `seat=<id>` — the id `seats.json` names as `spread`, one such frame per biome, at the tier being read, in L2b's frame step beside the `--seats all` frames — how the ruler frames of P5's and P6's gates are made
...
6. **`sim`** (`:sim`) — `sim <command> [flags]`: `battles`, `runs`, `selfcheck`, `spd-gate`,
   ...
   status, set bonus, sigil effect, pending kind, room type, ascension row, pact, forge mode and skill id that appears
...
10. **The review bundle** — `build/review/<sha>/`: the matrix, the test summary, the lane
    ...
    it — and what the owner judges on a phone is posted inline in the pull request body by
    the App (an actor's committed `sheet.png`, whose panels carry its lit frame, § F3.5; a screen's changed
    goldens side by side), since a linked artifact does not render there (on a public repository; under README question 8(b)'s private branch the committed sheet is opened from the repository in GitHub's mobile app).
...
- **The environment recipe**: `ci/env/` holds one recipe in two forms that must agree —
  ...
  mirror it), the Playwright browser (Chromium, at the version `versions.env` pins — every prototype capture and P4 contact sheet runs through it against the Vite dev server), the bundled fonts, warmed caches where the host allows — for a host that cannot
  run the image, on Linux with `apt` and on macOS with `brew` for the system packages alone — the JDK from an Adoptium tarball at the full version `versions.env` pins, Gradle from the wrapper's distribution and Node at `.nvmrc`'s exact version through `nvm`, on both hosts and under `setup.sh`'s own root, since a Homebrew or `apt` formula pins a JDK by its major version alone and `env-check` compares the patch — since the owner's Mac runs
  ...
  versions (the JDK, Gradle, Node, the Playwright browser, every Android SDK package `versions.env` names — the manifest's scope is what the recipe installs, at the versions found, under `setup.sh`'s own SDK root, never what a host already holds — the fonts, and a `marker`
  ...
  written by `setup.sh` at session start and again after an `env-image` hand-back, and the
  agent's host is not bare, so an edit to the recipe is unverified until L3's bare-runner job;
  ...
  created in § V9's M3), which runs on every `pull_request` — its build job under `if: github.event.pull_request.head.repo.fork == false`, since a fork head's token never holds `packages: write`, and an outside pull request is never merged (§ V5) —, its own `changes` job on the three recipe globs — `ci/env/env.Dockerfile`, `setup.sh`
  ...
  registry tagged by the head commit (a retention rule on the package keeps the pinned digest and the two newest tags, since each image is gigabytes), and uploads the rebuilt `manifest.image.json` and
  the digest as an artifact the agent commits into the same pull request — with the `kmp-*.yml` regenerated from the new `lanes.yaml`, or the generated-workflows job fails that pull request; the hand-back commit is followed by `setup.sh` on the agent's host, so L0's `env-check` compares the new manifest against a fresh host manifest instead of reporting red for the rest of the session — (the
  ...
  registry and rebuilds only when a recipe file changed since that commit, or when no
  commit-tagged image exists at all — spike 6's throwaway build pushes under the same
  commit-tag scheme, so M3's first run finds it — the manifest
...
- **Rendering**: goldens are recorded and compared **inside the image**. An agent whose
  ...
  **`record-goldens`** workflow — its own `workflow_dispatch`-only workflow with a
  `kind` and `name` inputs (screens, frames, vfx or backdrops; one or all), dispatched on the pull request's head ref, which the job checks out (dispatched on `main` it would render `main`'s screens and record nothing for the branch's new one), whose job names are outside
  ...
  takes a per-CPU-class tolerance on the goldens — per-channel |Δ| ≤ 2 on at most 0.5 % of a
  golden's pixels and no pixel above it, the recording run writing the CPU feature set
  into the golden's sidecar and the compare reading it, a loss recorded in the README's
  risk table since the gate is then no longer byte-exact — the one executable fallback, since Skiko ships a prebuilt `libskiko` and Skia's
  SIMD dispatch cannot be pinned without building Skiko from source, which is not scheduled — and outside the image (an agent session, a macOS or Windows
...
rules against the merge ref) — whichever copy of the file GitHub runs for that event, the base's or the merge ref's, the property rests on nothing the file holds — and it is still
...
| A visual regression in the lit frame (bloom, light, actors, fog, motes) | the biome frame goldens; the `backdrops` and `vfx` goldens; the art tool's rulers over the frames (gated in L3, § V2); the full-frame critic on the bundle | L2b · L3 · review |
...
| A performance regression on a phone; the tier drop not firing | the device benchmarks; the forced-slow-frame flow (under question 5's farm branch the tier drop is the owner's manual walk before each `v*` tag); a device lane that stops reporting files the nightly's issue after three nights (§ V7) | L4 |
...
| A spec clause nobody tests | the binder (the nightly's own run for the clauses only `Device`- or `Nightly`-tagged tests bind) | L1 · L2b · L4 |
...
| The plan, the rubric or the hooks rewritten by a writer | `plan/**` except `plan/spikes/**`, `.claude/**` and `CLAUDE.md` are owned paths | L3 |
...
device-runner repository's rows appended as comments to a standing `ledger` issue in this repository, which the owner creates at M2 — never the failure issue, which exists only on a red night (`TECHNICAL.md` § T13.5;
...
otherwise, naming the slowest task — and files it too when no `device-runner` row (or the farm's, under question 5's other branch) is newer than three nights, since a stopped device lane would otherwise report nothing rather than SKIPPED. Also tracked: repository size — the clone the freeze check's `fetch-depth: 0` pulls on every pull request and
`release.yml`'s on every tag, where the image goldens, the contact sheets, the
645 + 645 frames, the six backdrops and the 72 WAVs all live in history — first measured at
P1 and monthly after, and past 500 MB the remedy is a blobless partial clone on those two
checkouts (`filter: blob:none` — the commit count and the freeze check's comparison need the
history, not its blobs, which are fetched on demand for the paths read), not Git LFS, whose
allowance is one gigabyte of storage and of monthly bandwidth per account, public repository
or not, that every full checkout would spend at ≈ $5 a month per data pack; flakes (target
...
- **M2 — the review mechanics.** The owner creates the standing `ledger` issue (§ V7; the two `main` runs' re-dispatch log lands on it from M3), sets the repository's Actions settings (the default token read-only; Actions may not create or approve pull requests, `TECHNICAL.md` § T1; approval required before an outside collaborator's workflow runs; auto-merge allowed, § V5), creates `CODEOWNERS` and *then* the
  **provisional ruleset** (a pull request required, approvals 0, a code-owner review
  ...
  proves whether a code-owner review is enforced at zero required approvals — and if it is not, required approvals go to 1 until M4's owned-path check exists, then back to 0, so M2–M3 never rest on `CODEOWNERS` alone, and M1, which lands before the ruleset, on discipline audited by M4 (§ V1).
- **M3 — the rig's skeleton.** The convention plugins and the empty modules with the edge
  ...
  workflow, `record-goldens.yml`, `keepalive.yml`, and the owner-review workflow with its review-event re-dispatch (§ V5;
...
- **M4 — the gates.** The required checks added to the ruleset (every lane job and every
  ...
  generated-workflows job, the three owner-review checks from the gate App — the list `ci/required-checks.txt` the generator writes — the generated jobs' names plus the hand-written jobs' from `lanes.yaml`'s `required-hand-written:` table and the three gate-App checks from its `required-gate-app:` table, § T13.5 — which an L3 job checks against the branch's active rules from then on), the tag ruleset and the `release` environment with its `main`-and-`v*` deployment rule; Renovate installed → **the second
...
- **M5 — the analysers and the binder.** Spotless and detekt with the budgets → the
  ...
  missing report each fail a fixture lane) → Konsist and the JVM ABI dump → the verifier's stub generator (`:tools:stub`, § T6.3), an S inside P1's L → the `ART` clauses promoted to `contract` in the same pull request, bound by the art tool's gate tests and the calibration golden from P1 (§ T7.2's stateless rule would otherwise fail every pull request from here) — the three in-scene-bar clauses excepted: the LOW and MED bars' stay `proposed` until P5's `art rulers` step binds them, the HIGH bar's until P6's (§ V2's L2b), since a `contract` clause with no passing test in the lane's scope fails the binder —, under the owner's contract-clause review; the `BALANCE` clauses are P3's, promoted with the rules clauses once the reduced snapshot binds them.
- **M6 — the hello-world stage.** `Frame`, `Layout` and `Stage` drawing one fallback
  ...
  and compared on macOS (the owner's Mac, or a hosted-macOS job) — and one frame of the same stage from the iOS simulator, compared with the JVM's by `art gate`'s sheet metrics over both frames, at § T10.4's second-reader tolerances, the second iOS rendering after spike 1's —, where the step asserts
...
- **M7 — the hooks and the platforms.** The hooks (edit, pre-commit, pre-push) → the
  Android, iOS and wasm compiles in L3 → the nightly skeleton and, the standing `ledger` issue in this repository (created at M2, under either branch of question 5; the lane's rows and the two `main` runs' re-dispatch log land on it, § V7, § V5) and, if question 5 chose the runner, the private
  ...
  it — reading the frame time of M6's hello-world stage with both blend modes on, the first Android frame-time reading, recorded in the ledger and read at the P1 stop against the p95 ≤ 12 ms tripwire (README risks) — from the app and androidTest APKs L3 assembles and uploads from M7 on (P5's device lane reads the same artifacts), which the owner downloads by hand (`gh run download`; the token that automates it exists only once question 5 chooses the runner) — the guest assembles nothing, since it is
  ...
  development-only signing identity (§ T1) is created in the sitting that closes question 5, right after M7, under the host's separate
