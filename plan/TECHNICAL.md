...
- **Every workflow runs on every pull request; jobs skip themselves.** A required check
  ...
  and an `if:` that ORs its own module booleans with `build` and `root`, so a build-logic,
  catalog, wrapper or analyser-configuration change runs the whole lane (Renovate's batch is
  a `build`-only diff) — a job skipped by its
...
- `owner` names the module whose tests bind it and whose generated `commonTest` receives
  ...
  itself and whose tests report as JUnit XML the binder reads like any other task's; the owner per area, which is also the module whose `<Module>Fixtures` receives the area's fixtures (§ T2.1): `:core` for the rules areas, `SAVE` and `SYNTHETIC`, and for `spec/golden/fixtures.md` and `spec/golden/vault.md` — rules types, read by `:sim`'s harness and by `:tools:instruments`' storyboard, which may not depend on `:sim` — `:sim` for `spec/golden/cells.md` and `BALANCE`, `:engine` for `PLATFORM`, `:ui` for `SCREENS`;
...
`frames --bloom off`, so a prop glow or the sky body cannot move a reading), not over the
