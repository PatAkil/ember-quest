...
| `SUMMON` | `full=<0\|1> opening=<0\|1> offers=<id>:<favored 0\|1>,… dominant=<ELEMENT>` (`-` when `offers` is empty — reachable off the golden path with a one-entry `config.roster`, the roster quirk's clause; an offer carries no relic; the full-party EPIC card is rolled after the answer and shown as its own `RELIC` pending) | `take=<i>` or `swap=<i> out=<j>` or `-` |
...
`seats: list<string>` (six ids in anchor order — `H0`, `H1`, `H2`, `E0`, `E1`, `E2`, the anchors file's own naming, § T10.4) and `spread: string` — § T7.4's vocabulary
...
(gitignored, rebuilt in every lane), which the art tool and `cells.mjs` read instead of
...
(`FUNCTIONAL.md` § F4.3), each re-reading dated in `LICENSES.md`, a changed term the same
...
- **A run** is `(RULES_VERSION, seed, RunConfig, [debug], decisions[])` in `:core`'s canonical
  ...
  is out of range, restarts from that snapshot; an open room keeps its saved payload where
...
- **Store set-up (P5, before the first device build ships to testers)**: the App Store name,
  ...
  TestFlight's **internal** group — the owner's own install path, and all the first tag does (three to five of the lined-up testers join the internal track at the last scheduled stop, before the store set-up, README's P5 row),
...
job is not required (generated inert until M4's ruleset carries the list, the M3–M4 window § V1 names beside M2's `CODEOWNERS` one); adding one is the owner's act, named in every phase that adds a job
(P1's M7 — the Android and iOS compile jobs, one owner ruleset act inside P1's eight sittings; the wasm job is informative until P8 — 5b's arm64 leg at M4, generated with the merge lane's jobs when the spike passed and inside that act; P5's device jobs; P8's wasm) — the pull request that adds the job fails
