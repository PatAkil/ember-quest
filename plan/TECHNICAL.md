...
| iOS | product | iOS 16.0 | `iosApp/` Xcode project with a Swift entry point hosting `ComposeUIViewController`; no other Swift but the `PerfTests` XCUITest target (§ T9.5) |
...
(`ubuntu-24.04-arm`, free for public repositories — announced in January 2025, re-read at P0's terms check as the rates are) are where the arm64
...
the iPhone lane under a separate user account on the host, since it needs Xcode — with a
development-only signing identity, a `development` branch of the `match` repository holding
a development certificate and the phone's provisioning profile and never the distribution
material, created in the sitting that closes README question 5, right after M7, on the runner branch — the
...
daily quota that is the ceiling on device time and a lifecycle rule on the results bucket for its storage, since a Cloud budget only alerts (README question 5)
...
spikes that need macOS (spike 5's iOS simulator hash test and spike 1's lit iOS frame) run on the owner's Mac or on a
...
repository with `statuses: write`, `issues: write` and `actions: read` only — `contents: read` too under README question 8(b)'s private branch, for the checkout and the artifact download — held by the private runner repository to post the
nightly's result back, to append its ledger rows as comments to the standing `ledger` issue (§ T13.5) and to download the APKs of the latest successful post-merge L3 run on `main`, since the arm64 guest assembles nothing (README D18); and the **farm's Google Cloud identity** — a service account holding Test Lab's role alone, reached through workload identity federation from the nightly's OIDC token (no stored key; `id-token: write` on the nightly's farm job and on nothing else, the federation bound to this repository's `main`), which the owner creates with the Firebase project at P1 (README question 5) — under the runner branch too, since the fallback nights run the farm. Every agent session mints installation tokens on demand from the App's
private key (an environment secret the owner sets) through `ci/mint-token.sh`, git's credential helper — a token lives one hour, a session twelve to sixteen (§ T13.3) —, and commits, pushes and opens pull
...
throwaway pull request verifies it rather than assuming it. The repository's Actions settings are the owner's too, set at M2 and proven by M4's
throwaway pull request: the default `GITHUB_TOKEN` permission read-only, and "Allow
GitHub Actions to create and approve pull requests" off — the second of which caps any
workflow copy a head branch carries, `owner-review-redispatch.yml`'s included (a read-only default caps nothing, since a workflow's own `permissions:` block may elevate; the owner-review checks rest on the ruleset requiring the three checks from the gate App, which no workflow token can post)
(`VERIFICATION.md` § V5). A fine-grained
...
├── storyboard/            :storyboard  the storyboard driver (§ T6.2), main code with its own device and simulator test source sets
...
├── tools/art/             the art tool (TypeScript, Node; its own package.json and lockfile, `npm ci` a lane step of the jobs that run it — the asset gate, the calibration golden, `art rulers`, and `gate.sh` locally — cached on the lockfile hash, on the image's Node, never a recipe step, so a lockfile bump rebuilds no image and the manifest carries no entry for it; in Renovate's scope): generate · normalise · gate · rulers · sheet · accept — every image measurement
...
├── iosApp/                the Xcode project — the Swift entry point hosting `ComposeUIViewController`, and `PerfTests`, an XCUITest target whose `XCTMetric`s assert the iPhone's frame and memory budgets — the frame one a custom signpost metric over `:engine`'s per-frame `os_signpost` intervals (§ T9.5; P5)
...
├── ci/                    env/ (env.Dockerfile, setup.sh, versions.env, manifest.image.json), mint-token.sh (the App's on-demand token mint, git's credential helper, § T13.3), lanes.yaml, gate.sh, required-checks.txt (generated, committed)
├── config/                analyser configuration data: detekt, ktlint (owned); the Konsist suite itself is `build-logic`'s quality plugin
...
├── .github/               workflows: pages.yml (builds prototype/, publishes docs/ from P5; hand-written), prototype.yml (the freeze check and the prototype's gates; hand-written, outside the generated-workflows diff), env-image.yml (builds the environment image and re-checks its manifest; hand-written), owner-review.yml and owner-review-redispatch.yml (§ V5, hand-written), record-goldens.yml (`workflow_dispatch` only, its jobs outside `ci/required-checks.txt`; hand-written), keepalive.yml (a monthly `schedule` that pushes a dated commit to its own branch with the built-in `GITHUB_TOKEN` — intended as the deterministic repository activity, unconfirmed until the first quarterly session (§ V5), and no App key in Actions; hand-written, not required), release.yml (on a `v*` tag: builds, signs and uploads through the `release` environment, § T12; hand-written, outside the generated-workflows diff, a P5 deliverable), the generated kmp-*.yml — every hand-written one declaring its own `permissions:` block, since M2 sets the default token read-only: `pages.yml` `pages: write`, `id-token: write`, `contents: read`; `prototype.yml` `contents: read`; `env-image.yml` `packages: write`, `contents: read`, `statuses: write` (its `push`-to-`main` status, § V4); `owner-review.yml` `contents: read`, `pull-requests: read` (the checks go through the gate App's key); `owner-review-redispatch.yml` `actions: write`; `record-goldens.yml` `contents: read`; the generated `kmp-*.yml` `contents: read`, `actions: read` (the last nightly's conclusion), `pull-requests: read` (the re-run comment), `statuses: read` (the post-merge and `env-image` statuses on `main`) and `statuses: write` on the post-merge run alone; the image package is public on the recommended branch (made so by the owner after its first push, README's P1 row), and `packages: read` joins every job that runs in it under question 8(b)'s private branch; `keepalive.yml` `contents: write`; `release.yml` `contents: read`, `pull-requests: read` (the release notes), its signing material and store keys from the `release` environment alone, no `id-token` (the uploads use the stored keys, not OIDC); the nightly `issues: write`, `statuses: write`, `contents: read`, `actions: read` (the bundle artifacts), `pull-requests: read` (the ledger rows in pull-request bodies), and `id-token: write` on the farm job alone (§ T1); CODEOWNERS; renovate.json
...
|---|---|---|
| `:core` | Kotlin stdlib; `:core-testing` from its test source sets only (the SHA-256, the builders, the generated fixtures) | `kotlinx.*`, `androidx.*`, `java.*` beyond what stdlib maps, `android.*`, `platform.*`, any logger, any clock, `kotlin.random` |
...
| `:ui` | `:core`, `:engine`, Compose, compose-resources, `kotlinx.collections.immutable`; `:core-testing` from its test source sets only | platform APIs except through `:engine`'s `expect` surface |
| `:sim` | `:core`, `:core-testing`, kotlinx-serialization-json, a CLI parser | Compose |
| `:storyboard` | `:ui`, `:engine`, `:core`, `:core-testing`, Compose `ui-test` | platform APIs except through `:engine`'s `expect` surface — the storyboard's tap-and-assert driver in its `commonMain` (`runComposeUiTest`), run on the device and the simulator by its own `androidInstrumentedTest` and `iosSimulatorArm64Test` source sets in L4; a shipped module cannot depend on a test source set and a JVM-only one can drive neither an instrumentation APK nor a simulator test, so the driver is main code in a module of its own |
| `:tools:instruments` | `:core`, `:core-testing`, `:ui`, `:engine`, `:storyboard`, JVM libraries | — (the JVM CLI over `:storyboard`'s driver) |
...
iosSimulatorArm64 and wasmJs, plus a test-only `linuxX64` klib target for the verifier's stub, which `:core-testing` also carries (§ T6.3). `:engine` and `:ui` compile for the same set. Everything under
...
since the harness, the instruments, `:storyboard` (the driver) and the debug drawer read them, and the tables and the
...
`spec/fixtures/save/**` is excluded from the binder and read by JVM tests from resources)
and exposed as `<Module>Fixtures.<Area>.<FileStem>.<name>` — one root object per owning module, named after it — `<Module>Fixtures`: `CoreFixtures` (generated into `:core-testing`'s `commonMain`, the source set `:core`'s fixtures live in, § T7.1), `UiFixtures` into `:ui`'s `commonMain`, `EngineFixtures` into `:engine`'s test source set for the `PLATFORM` fixtures (its public API is a reviewed dump, § T3, as `:core`'s is — the same reason `CoreFixtures` lives in `:core-testing` and not in `:core`'s shipped `commonMain`) and `StoryboardFixtures` into `:storyboard`'s `commonMain` for `RUN-FLOW`'s — the generated file lands in the owning module's `commonMain`, or in its test source set where that module's ABI is dumped (§ T7.2) — typed by the area file's **first** `schema:` block, its root; every later block in the file is a nested type reachable only through `object:<Name>`, which resolves across the files of one `spec/<area>/` (so `spec/golden/vault.md` may nest the `VaultRelic` that `fixtures.md` declares), and a fixture that does not parse as the root is a binder error; and the golden hash lists as constants. Full-text goldens, screenshots and the save corpus are read by JVM-only
...
- Pending kinds are the prototype's — `DRAFT`, `VAULT_EQUIP`, `SUMMON`, `LEADER`, `ROUTE`,
  ...
  - `Forfeit` aborts the turn loop and books the battle as a lost battle (`deathKind`
    ...
    `choosePack` and a live battle only when present, bound by `PLATFORM-DEBUG-NN` clauses (§ T7.1's `PLATFORM` area, where § T14 and § V3.2 put the hooks) — a post-gate `:core` change made at P5 through § T7.7's route, as `resumeRun` is, its clauses first and the golden path proven unmoved — by `diff-oracle` while the oracle stands, by the committed golden hash lists (the unchanged cells, `RULES_VERSION` not bumped) once a post-parity rules change has retired it, § V8.3; and
    `BattleCtx.spdDelta` the fifth, a rule the goldens never set — the battles harness's
    diagnostic `--spd`, a flat delta on `base.spd` at battle creation — bound by a
    `COMBAT-SPD-NN` clause test; and the empty-roster fallback the sixth — `config.roster` empty falls back to `ROSTER` in `runSteps` while a landmark SUMMON reads the raw field and the opening one the fallback (§ T5.2), which no golden cell sets, since the harness always passes the explicit six and the cell table has no `roster` column — bound by its `RUN-CONFIG` clause tests alone; and `RunSession.decide`'s refusal of an answer whose type does not match the open pending the seventh — the prototype's `createRun` passes any answer through and lets each decision's own fallback decide — bound by a `SESSION` clause test alone; and the lap cap at 32, the lap the `pow` table's last exponent serves, the eighth (§ T5.2), where the prototype's lap is unbounded; with `minAscensionFor`'s vector and `resumeRun`) and is bound by its `COMBAT-FORFEIT`
    clause tests alone.
- Answers are a sealed `RunAnswer`, one type per pending kind, integers as `Int`; an
  ...
  decides it, as in the prototype; an answer whose type does not match the open pending
  (`Turn(0)` at a `DRAFT`) is refused by `decide` — it returns false and nothing moves —
  which the `SESSION` clause tests bind.
...
class RunSession(config: RunConfig, rng: Rng) : Run {
    ...
    override fun decide(answer: RunAnswer, expected: Int?): Boolean {
        if (busy || pending == null || !answer.matches(pending!!) || (expected != null && expected != token)) return false
...
**The three-way self-check** has two genuinely different mechanisms and a host-protocol check where the prototype's
...
outside the session; (c) `RunSession.decide` fed by `answerWith` through a `HeadlessRunHost` in `:sim` that reproduces `:ui`'s token protocol — a host-protocol check on (b)'s
mechanism, not a third mechanism (the storyboard driver exercises
...
|---|---|
| The plane split | far · mid · floor · near at depths 0.3 / 0.62 / 0.9 / 1.35; blur 6 / 1.2 / 0 / 8 px, the floor crisp because it is at the actors' depth; everything blurred is baked once per (biome, tier); the near plane is drawn behind the actors, as the prototype's is, so a near-plane element is never authored where it would have to occlude a body — the curtains ramp out before the first seat and the lip stays below the front rank's feet (P6's painters, `FUNCTIONAL.md` § F3.7) |
| The light on the ground | two foot pools **derived from the stage anchors** of the two ranks, each rank's `rx` alone scaled by one factor — `ry` stays the rank's half-height plus its pad — until the pair overlaps by at most 60 px (the prototype's `backdrops.ts` does exactly this; its own comment and `CLAUDE.md` say "both radii", and § T4.2's `rig.json` export records the code's rule, not the comment's); key and fill lights with a per-body weight so the rig favours neither rank; the shafts baked **into the light map** (the prototype blits one light map per frame with a breathing alpha), fog banks and dust motes as animated composites behind the actors |
| The light on a body | a multiplicative gain (`ColorDodge`) under a feathered ellipse, tested at the feet and spread-capped to the median seat; a small additive rim spill; contact shadows with a cast lobe; the bloom never on a body — the constants of all of it (`GAIN_*`, `RIM_*`, `SHADOW_CAST_*`, `VIGNETTE_*`, the bloom's) are the prototype's, exported once as `rig.json` (§ T4.2) and bound as `spec/platform/` constants through its committed copy `spec/fixtures/platform/rig/rig.json` |
...
| The frame and the input | 1280×720 logical, a mutable safe inset, `TAP_MIN` 96 with drawn rects tested first and each actor's rect clamped to its own cell (the midpoint split between neighbouring feet, `spriteCellX`/`rankFrontier`), the ±50° focus cone, A/B/PAUSE with digits for skills |
| The stage anchors | `HERO_FEET`, `ENEMY_FEET`, `ENEMY_FEET_PAIR` and `BOSS_FEET` exported with `rig.json` (§ T4.2) and bound as `spec/platform/` constants — the landscape stage's six seats, which the pools derive from and the in-scene bars are read at, and the two-enemy and boss seatings every pack and boss battle stands on |
...
| detekt | 1.23.x stable, 2.0 when it leaves alpha | smells, complexity, custom purity rules, Compose rules | syntax-only rules in L0 on changed files; type resolution over the tree in L2a, never in a fast lane. **Fallbacks, distinct**: if detekt cannot *parse* the Kotlin 2.4 syntax `:core` uses, detekt is dropped — Konsist enforces function length and parameter count **and takes over the custom rule set** (§ T8's purity, readback and sleep rules are import and call checks Konsist can express), the complexity budgets, the Compose rules and detekt's smell set (`!!`, `lateinit`, return counts, nesting) are suspended and the loss is recorded in the README's risk register; if only type resolution is too slow, the syntax-only rules stay and type resolution is dropped |
...
| actionlint, shellcheck, markdownlint, a custom spec-lint; `html-validate` and `lychee` (offline, the internal links) over `docs/**`, `lychee` over `plan/**` | current | workflows, scripts, documents, the spec, the privacy and support pages, the plan | L0; the two pages and `plan/**` in L3's lint job |
| Renovate | — | one **monthly** batch pull request for dependency bumps, through the full lanes, reviewed by the owner like any change to the build; its configuration is `.github/renovate.json` (an owned path under `.github/**`, in the `build` boolean), written at P1, never by Renovate's own onboarding pull request, with `prototype/**` in its `ignorePaths` so the frozen tree gets no bump outside § T4.2's list | the catalog owns library and plugin versions, `versions.env` the toolchain (the JDK, Gradle, Node, the Android SDK packages, the Playwright browser) with a lint that pairs the JDK and Gradle entries of the two and the wrapper's `distributionUrl`, and the browser pin with `prototype/package-lock.json`'s Playwright version (`gradle/**` and `gradlew*` are owned paths, § V5) — a lint that runs unconditionally in every lane, since its files sit in four `changes` booleans (`env`, `root`, `prototype`, `build`) and it costs seconds; `tools/art/package.json` is in Renovate's scope too |
...
root `.gitignore`, `ci/mint-token.sh` (the App's on-demand token mint, git's credential helper, § T13.3 — the move commit is the first push it signs), `ci/env/versions.env`
...
- **The prototype is frozen.** `prototype/**` is code-owned from P1; the only changes ever
  ...
  ("frozen prototype — the game is being rebuilt, see `/plan`"), and `prototype/STATUS.md`'s header,
...
- **Every workflow runs on every pull request; jobs skip themselves.** A required check
  ...
  changed against the merge base and outputs booleans — `prototype` (`prototype/**`), `plan` (`plan/**`, ORed by the lint job with `docs` — markdownlint and `lychee` offline over it before merge), `spec` (`spec/**`), `assets` (`assets/**`), `art` (`tools/art/**`), `env`
  (`ci/env/**`), `docs` (`docs/**` — ORed by the lint job, whose HTML and link check over the two pages is their one pre-merge reading, § T8), one per Gradle module — `core` (with `core-testing/**`), `engine`, `ui`, `storyboard`, `sim`,
  ...
  and an `if:` that ORs its own booleans — the module booleans of the tasks it runs; `art` (with `assets`, `engine`, `ui` and `tools`) on the asset gate, the calibration golden, its second reader and `art rulers`; `engine`, `ui`, `core` and `tools` on the screenshot, frame, `vfx` and `backdrops` goldens, which render `:engine`'s rig from `:tools:instruments`; `ios` (with `app`, `ui`, `engine` and `core`) on the iOS build and the simulator boot smoke; `env` on `env-check` — with `build`, `root`, `spec` and `assets`, so a
  build-logic, catalog, wrapper or analyser-configuration change — or a spec-only or
  asset-only pull request, the fold, a clause promotion, a fixture edit — runs the whole lane
  (Renovate's batch is a `build` and `art` diff, the art tool's `package.json` being `art`'s) — a job skipped by its
...
- `prototype/tools/capture.mjs sheets frames=1` (the tool gains `frames=1` — `sheets` reads `actor=` alone today — and `out=<path>`, resolved against the repository root — its `OUT` is hard-coded against the working directory today; and a new export face on `lineup.ts`'s `window.__lineup` — per bake, a data URL of the cell-resolution bitmap, its `feet` and the derived `hitRect` — named here as `stage.ts`'s anchors and `sfx.ts`'s `render` are, since today's `__lineup` exposes `{ready, sheet, mode, zoom, metrics, humanoids}` and no per-bake bitmap): writes every actor's fifteen bakes as
...
- the `flat=1 tier=<LOW|MED>` frames, for the LOW and MED bars: the biome's unlit composite
  ...
  `capture.mjs battle biome=<BIOME> flat=1 tier=<LOW|MED> seat=all anchors=1` — the
  stage-only `battle` of the `PixelActor` bullet below, no `pixel=`, the fallback cast, the
  36 seats and their masks —; the HIGH bar's frame is the prototype's own four-plane diorama,
  `capture.mjs battle biome=<BIOME> tier=HIGH seat=all anchors=1` with nothing flattened,
  the frame P6's four landed planes are measured against, and a `flat=1 tier=HIGH` frame
  set is captured too, with no bar of its own, for P6's equivalence check at HIGH (§ T10.4);
  and one small engine hook — a flat plane supplied as an image: at LOW, `bakeFlat`'s key
  light and grade painted over it in place of its painter loop; at MED, the rig over the
  flat blit of it; at HIGH, the HIGH rig — the light map, the gain, the spill, the highlight
  lift, the bloom off — over the same flat blit, the `flat=1 tier=HIGH` set's frame — which no tier does today, since the engine lights only planes it baked
  itself — and, in the same pull request, a `bloom: false` option on `createLight` that
  `renderPost` honours, which the MED frame passes — and the HIGH diorama and the `flat=1 tier=HIGH` set too, since the bloom lifts the ground strips hardest at HIGH and the Kotlin ruler frames they are compared with run `--bloom off` —, since the prototype's bloom has no
  switch today: two of the four engine changes P0 makes to the prototype, named here as
  allowed changes (the third is the `unlit` flag below, the fourth the rig constants' lift
  and export in the `rig.json` bullet);
- `prototype/tools/capture.mjs backdrops flat=1`: an `unlit` flag on `engine/light.ts`'s
  **bake** path (`bakeFlat`, which at LOW paints the key light and the grade's multiply
  ...
  one placeholder plane per biome for every tier the stage offers — LOW and MED at P5; HIGH is
  enabled with P6's planes and its bar (`FUNCTIONAL.md` § F1.3), the `flat=1 tier=HIGH` set serving P6's
  equivalence check alone — (the near painter's curtains and lip are
  ...
  `SHADOW_CAST_SQUASH` and the 3.0× lobe, `VIGNETTE_RX/RY`, the bloom's alpha and divisor, the contact ellipse's radii and alphas, its ink `SHADOW_INK` and the cast lobe's offsets, `RIM_PUSH_X/Y` and the `GLOW_*` set),
  bound as `spec/platform/` constants (§ T2.5) — the same pull request commits the export's copy as `spec/fixtures/platform/rig/rig.json`, typed by a `schema: Rig` block in `spec/platform/rig.md`, so the binder generates the constants as it generates every fixture (§ T2.1), and a lint holds the two files equal —; the named constants are module-private `const`s in `engine/light.ts` and `backdrops.ts` today, and the contact ellipse's radii and alphas and the cast lobe's offsets are literals inside `drawContactShadow`, so the same pull request lifts those into named module constants and adds `export` to every one, as the ramps export does —, serialised in declaration
  order (`id`, then key and fill with their `actorWeight`, the pools' colour, ellipse, alpha and `actorWeight`, the grade, the fog,
  the motes, the shafts, the sky body, the rim colour, the ambient preset and colour), plus
  `anchors: {hero: HERO_FEET, enemy: ENEMY_FEET, pair: ENEMY_FEET_PAIR, boss: BOSS_FEET}` from `layout.ts` (the two-enemy packs and every boss stand on the last two) — bound as `spec/platform/`
  constants P5's `Layout` uses for the landscape stage, so the P0 bars and P5's readings
  share one geometry, and a seat that moves re-records the bar as an accepted miss does
  (§ F3.1) — and
...
- `prototype/tools/render-sfx.mjs`: before boot it installs an `OfflineAudioContext`-backed
  ...
  through `prototype/tools/sfx.html` with its `sfx.ts` (under `npm run check` like every page in `tools/`), a fixture page that imports `createAudio` from the
...
- the **`PixelActor` registry** (`prototype/game/art/actors.ts`) and `capture.mjs battle
  [pixel=<dir>] biome=<BIOME> party=<ids> pack=<ids> (`<BIOME>` the underscored token of § T7.5, `EMBER_CRYPT`, since the prototype's biome names carry spaces) [tier=HIGH|MED|LOW] [flat=1] [phone=1] [seat=all|<id>]
  anchors=1` (the fallback recipes stand in for every id when `pixel=` is absent, as the bar and `flat=1` frames need): with `biome=` present, `battle` leaves today's run drive for a stage-only capture in any biome and tier over `prototype/tools/stage.html` (below), and bare `battle` keeps the run drive — `<dir>` holds
...
- the **banner commit** of § T4.1 (the documents' banners and `prototype/STATUS.md`'s corrected
  header);
- `prototype/tools/export-ramps.mjs`: the five element ramps (`ELEMENT_RAMPS`) and the
  ...
  frozen paths; `parts.ts` holds the `Ramp` type, not the ramp values) — to `spec/fixtures/art/bible/ramps.json` — a fixture typed by a `schema: Ramps` block — `elements: list<object:ElementRamp>` (`element: string`, `accent: list<string>` and `glow: list<string>`, seven hex strings each, the oracle's `Ramp`) and `neutrals: list<object:NeutralRamp>` (`name: string`, `ramp: list<string>`) — that lands with this export in
...
- `prototype/tools/stage.html` with its `stage.ts`: a fixture page that draws bodies on the lit stage outside
  a driven battle — `biome`, `tier`, `flat`, `pose`, `tick`, `party`, `pack`, `seat`, `pixel` and `anchors` as query parameters, one for one the flags `capture.mjs battle` passes through by name (`phone=1` is the tool's own viewport; `pixel` a directory `capture.mjs` reads in Node — it and `spec/fixtures/art/seats/seats.json` lie above `prototype/`, which the prototype's Vite root cannot serve after § T4.1's move — handing the candidate PNGs and the seat list to the page as data URLs through Playwright, so no `vite.config.ts` edit is needed; `anchors` exposes the six anchors and the per-seat masks on `window.__lineup` for the tool to write) over
...
- optionally, under question 3(c): the three battle-screen defects (two root causes) fixed in
  ...
  request under the prototype's own gates, verified by three assertions added to `capture.mjs`
  in the same pull request — this list's one more prototype change, an S inside 3(c)'s
  session — one per § F1.4 battle-screen defect: `playfull` records beside each BATTLE
  answer the option the screen committed and the skill the CAST event fired and fails when
  they differ (the pre-tick enumeration); it compares each rendered command row's enabled
  flag against the post-tick `actOptions` and fails on a greyed legal skill (the cooldown-1
  greying); and a seeded Vault with three VIOLENT pieces equipped — the pull request writes the `localStorage['ember-quest/vault']` seeding itself (`version: 1`, `unlockedAscension` 3, `vaultSlots` 3), which P2's `strong=1` later generalises — (three is the Vault's
  cap, § T5.1) and the fourth taken in the run from a RELIC card the chosen `seed=N` is
  known to offer drives one battle through the extra turn, which must resolve without a throw
  (the extra-turn crash) — the first two read through `__eq` (its `run` and `view` faces
  extended with the battle's last committed option, its command rows and its last CAST
  event); each failing before its fix and passing after — at `seed=N` for the first two, the run
  diverging once the right skill fires, so what is required is the failure before and the
  pass after, not one run; the third forcing its own setup (the seeded Vault and a RELIC
  card it names through `__eq`) and needing only a throw before and a clean resolve after — the headless `sim` cannot
  show a screen defect, and `playfull-decisions.json` records answers, not skills (an
  unverifiable fix is reverted before the baseline play);
- if the demo proves unusable on the owner's phone at the baseline play (audio unlock,
  ...
  recorded, the owner's choice — a baseline not taken on a phone names the felt rows it cannot
  bar (`FUNCTIONAL.md` § F1.5.3), which are then judged on the `kmp-quality` rubric alone;
- a repair to the prototype's build, lockfile or CI when `prototype-check` breaks on a
  ...
  after P2's freeze, outside the rules paths, in a named pull request, and after the oracle
  retires the demo's build alone, out of the quarterly session (README question 3(a));
...
- `prototype/tools/export-study.mjs`: a Node rasteriser over the study's grid strings and
  ...
  `ember-study-x4.png` at ×4 nearest, with the sidecar of § T10.9 — its `element: FIRE` (EMBER's, from `CHARACTERS`), `pose: idle` and `frame: 0` written by the exporter, since `PixelStudy` carries neither the element nor a frame index (the pose is its `poses` key).

**At P2**, in this sequence: (1) the harness gains `--trace` and `--cell <id>`, the slug the `cell` line carries (§ T5.3; built on the exposed
`runSteps`, `answerWith`, `createBattle`, `nextReady`, `runTurn`, `isOver`, `battleOutcome` and `Battle.events` — plus one `export` on `applyAwaken`, the module-private function that swaps an awakened skill's id onto a cloned `CharacterDef`, since `PartyMember.awakened` alone changes no skill and the fixture reader calls it for an `awakened` hero — the one rules-path edit of the sequence, a bare `export` in `prototype/game/sim/run.ts` that changes no behaviour, landing before the freeze at (3) —, so
nothing else under the rules paths changes), `--ascension <A>` into `RunConfig`, `--path a|b`
to choose the seam path a `runs` trace follows — a is `simulateRun` (the generator body,
`viaSteps`), b is `runstep.ts`'s `createRun` (the interactive seam); path c, `HeadlessRunHost`, exists only in `sim` —, a canonical `--dump` (§ T5.4), a
strong-party battle fixture (the harness's three kindled relics on the fixture party — the row's `relics` are the same three `spec/fixtures/golden/vault/default.json` carries, written inline as § T7.4's schema types them, the P2 recording asserting the two agree) for
...
block of `spec/golden/fixtures.md`, which `prototype/sim/fixtures.mjs` reads at run time (resolving `spec/` against the repository root it finds above `prototype/`, whatever the working directory; under README question 8(a)'s new-repository branch the fixtures stay under `prototype/sim/` and the game reads them through the submodule)
...
`battle-fixtures`; `--no-stall-gate` lifts the harness's `STALL_MAX` exit in both modes — the runs mode has
the same 0.5 % gate over its stall rate, so a ten-run cell with one stall would fail
without it, as would the stall fixture and any 20-battle cell with one stall; a
harness-wide flag, listed here only because the prototype's help text puts it with the
battles knobs — golden recording and `diff-oracle` pass it in both modes; `--fixture`
...
`prototype/sim/fixtures.mjs`, the fixtures' run-time reader (§ T5.3), and `prototype/sim/coverage.mjs`, the
...
`localStorage['ember-quest/vault']` before boot with a `VaultSave` carrying `version: 1`
(`VAULT_VERSION` — without it `loadVault` hands back the empty Vault and the flag is silently
a fresh A0 run), `unlockedAscension` at least 3, the three kindled relics and
...
If the rebuild is abandoned at any phase, nothing has to be undone but a quarter-session rename: the prototype's demo builds and
deploys from `prototype/` exactly as it did (after P2 its harness reads the tuned fixtures from `spec/fixtures/`, so an unwind that deletes the new tree copies them back under `prototype/sim/` or retires the harness with the oracle — the demo is unaffected), its documents and skills are intact under
their banners — and its development loop comes back by renaming `prototype/.claude-archive/`
to `.claude/` again and restoring the prototype's own `CLAUDE.md` at the root (or by opening
the session with `prototype/` as its primary directory), about a quarter of a session, since
the move commit lands before the go or no-go (§ T14) —, and the ruleset, the App and the private runner repository can stay or be
...
clauses from `prototype/game/screens/vault.ts`, a table small enough to need no exporter) → `codec` (the trace, decision-log and snapshot encodings, § T5.3) → `session` (the three-way self-check) → the policies (their answers
are in the traces).
...
| `Math.pow(LAP_MULT.*, lap − 1)` | a **table** of the oracle's own `Math.pow` results for the three bases at exponents 0–31, generated on the pinned Node and committed as data with its test vector; an exponent past the table is unreachable because a `RUN` clause caps the lap at 32, the lap the table's last exponent serves — the prototype's `ctx.lap += 1` is unbounded, so this is the eighth seam divergence, the eighth § T2.3 names, unreachable in play under `LAP_MULT`'s scaling and bound by that clause's test alone | V8's `pow` is fdlibm's, not correctly rounded for exponents ≥ 3; a multiplication loop matched it on every value tried (27 of 27) but is not guaranteed to; the table is exact by construction |
...
| `config.roster` empty → `ROSTER` in `runSteps`, but a landmark SUMMON (`resolveSummon`) reads the raw `config.roster`; the opening SUMMON reads the fallback | reproduced as is | a quirk of the oracle (a reviewer measured 0/200 wins on an empty roster against 30/200 on the explicit six); a `RUN-CONFIG` clause names it as contract until the first post-parity rules change; the trace prints the config **as passed** |
...
nothing about ART. The cross-platform hash test — 20 seeds × three of the nine run policies (`random`, `balanced` and `control`) in `runs` and the two act policies
(`random`, `balanced`) in `battles` — 60 runs vectors and 40 battles vectors — hashed with a hand-written SHA-256 in `:core-testing` (about 120 lines of stdlib
Kotlin, checked against the NIST vectors and, on the JVM, against `MessageDigest`) against
a committed expectation — `spec/golden/hash.md`'s `data: HASH_VECTORS` table (seed, policy,
mode, `n` — 10 runs or 20 battles a vector, the hash taken over all of them — and hash) under a `GOLDEN-NN` clause, generated into `commonTest` as every `data:` table
is (§ T7.1), recorded at P3 and re-recorded only under § T7.3's golden rules — runs as a JVM test, as an Android device
...
| `cell …` | for runs: `cell id=<slug> seed=<uint32> policy=<name> runs=<N> asc=<A> vault=<n> spd=<d> path=<a\|b\|c>` — a cell is N runs on **one** rng stream in run order, as the harness runs them, and the golden path is **b**; for battles: `cell id=<slug> seed=<uint32> pack=<enemy ids '+'-joined> act=<n> asc=<A> clears=<n> party=<fixture> policy=<name> n=<N>` (the boss fixture's display name `BOSS HOLLOW_KING` is never a value: every field and the slug are space-free) — N battles on one stream seeded as stated (the P2 recording uses seed 1 unless the cell table says otherwise); the slug is the golden file's name and `diff-oracle` regenerates a cell from this line and the fixture row it names (or `default.json`, for a runs cell) |
| `config` (runs mode only; battles have no `RunConfig`) | every `RunConfig` field **as passed**, in this order: `ascension`, `vaultSlots`, `roster` (empty allowed), `spdDelta`, and each Vault relic in the compact relic encoding (the declaration's order puts `vault` second; the record's order is the one above) |
...
| `debug <hooks>` | saves only (§ T11), never in a trace: a storyboard's forcing hooks — `act=<n> lap=<n>` (applied before the first decision) and, each with its firing point, `room@<act>.<stage>=<TYPE>`, `pack@<battle k>=<ids>` (`+`-joined, as the `cell` line), `hp@<decision i>=<slot>:<n>`, repeatable per slot at one index (applied when the replay reaches that act's stage, that battle or that decision index — the party is empty at the first decision, so a point-in-time hook has no earlier moment), any of them absent — which `sim replay` applies at those points; a save carrying it is not portable across `RULES_VERSION` (`VERIFICATION.md` § V3.2) |
...
| the **compact relic encoding** | wherever a relic appears (`config`, `party`, `result`, the pendings and answers): `<SLOT>:<RARITY>:<SET or ->:<level>:<K or ->:<MAIN><base>:<SUB><value>/<rolls>,…:<SIGIL or ->` — `<base>` is `Relic.main.base`, never the derived `mainValue()`, which the level implies; the key of `<MAIN>` and `<SUB>` is the leading run of `[A-Z_]` and a relic value is always integral, so the split needs no separator; fields `:`-separated, substats `,`-joined in roll order each with its roll count (rules state: a recast re-rolls a substat `rolls` times), values by the integer-or-bits rule, `-` for absent; a list of relics `;`-joined, `-` when empty; the relic's id is never printed |
| `event <KIND> <fields>` | one per `BattleEvent`, fields per kind: `TURN_START actor enraged` · `TURN_END actor` · `CAST caster skill targets` · `HIT attacker target dealt absorb crit glance killed` · `STATUS_APPLIED\|RESISTED\|EXPIRED target status turns` · `HEAL target amount source` · `ATB_CHANGE actor delta(bits) reason` · `COUNTER actor target` · `DEATH actor` · `BURN_TICK actor amount` · `VEIL actor` · `STALL` — actors as `H<slot>` / `E<slot>`, skills and enemies by id; `ATB_CHANGE`'s `reason` is a closed, space-free union in `:core` (`trip skill render nemesis mending surge opener`, the oracle's string values), and the codec clause and spec-lint reject a value with a space, as the `cell` line's `BOSS HOLLOW_KING` rule does |
| `result` | for runs, exactly the `RunResult` fields the oracle's `types.ts` declares, in that order, except `probes`, which the `probe` records carry; for battles, `BattleResult`'s `won stall enraged actorTurns` only — its `probe` is the `probe` record and its `party` the closing `party` record above; a field the Kotlin adds (`forfeit`) is never printed; list fields joined with `,`, heroes separated by `;`, `-` for null, **for the empty string and for an empty list** (`awakened`, `rests`, `shrines` on a run that met none) (`deathBy` and `deathKind` on a win — no string field is ever legitimately empty and distinct from absent), `<PACT>:<0\|1>` per shrine offer in roll order (`RunResult.shrines` carries the declined ones too); relics in the compact encoding |
...
| `SUMMON` | `full=<0\|1> opening=<0\|1> offers=<id>:<favored 0\|1>,… dominant=<ELEMENT>` (both `offers` and `dominant` print `-` when `offers` is empty — reachable off the golden path with a one-entry `config.roster`, the roster quirk's clause; an offer carries no relic; the full-party EPIC card is rolled after the answer and shown as its own `RELIC` pending) | `take=<i>` or `swap=<i> out=<j>` or `-` |
...
| `FORGE` | `worn=<compact relics> options=<worn index>:<MODE>,… pool=<sets> levels=<n> rebrand=<sets per worn relic, ';'-joined>` | `relic=<i> mode=<MODE> substat=<k or -> set=<SET or ->` or `-` |
| `ALTAR` | `candidates=<indices>` | `awaken=<i>` |
| `HERO_TURN` | `actor=H<slot> options=<skill>:<target>,…` — `<skill>` the `ActOption` skill index into `def.skills`, `<target>` the target actor's own slot (`Actor.slot`) for an `ENEMY` or `ALLY` spec, enumerated over living actors ascending, `-1` for every other spec | `option=<k>` |
...
`prototype/sim/cells.mjs`, the P2 driver that walks every cell through `run.mjs --trace` (which writes only the canonical records to stdout, every report line going to stderr; `cells.mjs` redirects stdout — `run.mjs` keeps its `--flag value` grammar, the capture tools their `key=value`)
...
leader, the heroes with each one's `awakened` flag and worn relics, the lap and the pacts** — so the lap and the pacts come from the row alone — there are no `--lap` or `--pacts` knobs — and the `battle-fixtures` literal means lap 1 and no pacts — a `party` value names a row of
...
from its line plus its fixture row (a battles cell; a runs cell from its line and `default.json`), never from the line alone, so the two enumerate the
same set and `diff-oracle` regenerates a cell from its `cell` line and the fixture row it
names, or from `default.json` for a runs cell:

| Cell | Runs / battles | Purpose |
|---|---|---|
| `runs`: seeds {1, 2, 3, 7, 4242} × nine policies × 10 runs | 450 | the baseline |
| `runs`: `balanced` at A0, A5, A10 × `--vault 0/3` × `--spd 0/+10/−10`, seed 1 × 10 runs (the A0 · vault 0 · spd 0 cell is row 1's seed-1 `balanced` cell, counted once) | 170 | ascension rows, the Vault, SPD deltas |
...
| `battles`: a **stall fixture** (a party that cannot kill — healers under a VEIL boss at A10 — run to `TURN_CAP`), tuned at P2 until a `STALL` appears; if none appears within 200 tunings, `STALL` is recorded as unreachable in its `GOLDEN-NN` clause and exempted from the coverage gate | 20 | `STALL`, which no other cell produces (0 in 1 000 runs; the contract's stall rate is 0.0–0.1 %) |
| `battles`: a **set-and-sigil family** — the strong party wearing each of the eight 2-piece sets at two pieces and each of the eight 4-piece sets at four (a set has one bonus at one piece count), and each of the twelve sigils once un-kindled and once kindled (every sigil declares both effects), against the act-1 boss at A0, `balanced` × 20 | 40 × 20 = 800 | every set bonus and every sigil effect, which the run cells cannot guarantee |
...
set bonus, sigil effect, pending kind, room type, ascension row, pact, forge mode and `SkillId` that **appears** in
...
relic with it worn — once un-kindled and once kindled, since each sigil has both effects; an ascension row when a `cell` line carries it; a skill when a `CAST` line names it — the six awakened variants reach the set only through an ALTAR awakening, so an awakened-party battles fixture is the cheapest cell; a pact when a `pending SHRINE` answer takes it and a forge mode when a `pending FORGE` answer chooses it (the six pacts reach the pack scaling, the hero stats and the card counts, a pact appears too when a cell's fixture row names it in `pacts`, which is how the stall fixture guarantees VEIL, since a battles cell raises no pending; both classes under the same bounded-search exemption). Three kinds never
...
tests bind them (§ T2.3). The fixtures — the leader seat, the heroes with their `awakened` flags (the route to the six awakened variants, § T5.3's coverage) and their own worn relics, the lap, the pacts (§ T7.4's `GoldenFixture`) — live in `spec/fixtures/golden/fixtures/*.json` (§ T4.2),
...
and a cell is added until it does — with `STALL`'s route for any kind: after a bounded search
(200 tunings, as for the stall fixture) a kind that no cell reaches is recorded as unreachable
in its `GOLDEN-NN` clause and exempted from the gate, so the stop gate has a bound. Storage: `spec/golden/<cell>.sha256` holds one
...
`--cell <id> --policy --seed --runs --n --spd --vault --ascension --path --json --dump` (`--cell` is the slug the `cell` line carries — both harnesses take it and `cells.mjs` passes it, so the tracer writes the line and the driver never invents it — a label for `trace`, and for `diff-oracle` and `trace-hash` the selector that looks the cell's parameters up in `cells.md`; `--path a|b|c`
...
(`--clears 0`; `--no-stall-gate` is harness-wide, and golden recording and `diff-oracle` pass it in both modes). `--dump` is the
...
| Table-driven | `:core` | the spec's tables (enemy scale per act, roll ranges, loot weights), generated into `SpecTables.kt` by the binder, the test that walks each table hand-written per module at P3 (so a folded `data:` clause is unbound at P2, as the fold intends); untagged, so they count as writer tests | L1 |
...
| Storyboard | `:storyboard` (the driver, its own device and simulator test source sets), `:tools:instruments` (the JVM CLI over it) | skip-playback: every biome, boss and screen reached with the strong party and the forcing hooks, once by tag and once by keyboard; a two-act run to a KO (L2b); full playback with frames (L3); Android and iOS, two acts (L4) | L2b · L3 · L4 |
...
from the writer's branch, its **klib** ABI dump the input — the merged klib dump the ABI tool builds on `agent-env` — from a test-only `linuxX64` klib target kept on `:core` and `:core-testing` whatever question 3(b) answers, since the Apple klibs need a Mac and the wasm klib goes with 3(b)'s drop, and a stub cut from the committed dump alone would stop seeing the writer's new API —
...
weak clause does not count toward P3's "matrix 100 %" — except a clause that is only a
`data:` table, bound by the module's hand-written table test alone (§ T6.2) and exempt, since the table is
the specification and a second assertion of it would restate it. The critic role (an agent with eyes)
...
| `:sim`, `:storyboard`, `:tools:*`, the art tool | ≥ 60 % | — | — | — |
...
- Ids follow `AREA(-TOPIC)?-NN`, the regex `^[A-Z]+(-[A-Z0-9]+)?-[0-9]{2,}$`. Areas, each
  ...
  (`GOLDEN-03`), `SAVE` (`SAVE-02`), `RNG` (`RNG-01`, the stream and `pick`, `weighted` and `chance`), `SESSION` (`SESSION-01`, `RunSession`'s token protocol), `CODEC` (`CODEC-01`, § T5.3's text encoding), `SYNTHETIC` (`SYNTHETIC-01`, P1's synthetic module, retired with `status: retired` — never deleted, since ids are never reused — when the module goes in P3's first `:core` commit). Ids are never renumbered or reused; a retired clause
...
- `owner` names the module whose tests bind it and whose generated `commonTest` receives
  ...
  (§ T7.8) and whose tests report as JUnit XML the binder reads like any other task's; the owner per area — and a fixture is routed by its spec file, to the `<Module>Fixtures` of the `owner` of the file whose first `schema:` block types it (§ T2.1), which is how `RUN-FLOW`'s reach `StoryboardFixtures` while `RUN`'s reach `CoreFixtures`, and `PLATFORM-DEBUG`'s `CoreFixtures` while `PLATFORM`'s reach `EngineFixtures`: `:core` for the rules areas, `RNG`, `SESSION`, `CODEC`, `SAVE` and `SYNTHETIC`, and for `spec/golden/fixtures.md`, `spec/golden/vault.md` and `spec/golden/hash.md` — rules types, generated into `:core-testing` rather than `:core`'s shipped `commonMain` and ABI, read by `:sim`'s harness, by `:storyboard`'s driver and by `:tools:instruments`' storyboard CLI through the `:core-testing` edge, since neither may depend on `:sim` and `:core`'s shipped ABI carries no fixture (`:ui`'s `commonMain` does carry `UiFixtures`, for the previews and the debug drawer) — `:sim` for `spec/golden/cells.md` and `BALANCE`, `:engine` for `PLATFORM` — but `:core` for the `PLATFORM-DEBUG` clauses, which bind `RunDebug` in `:core` (`owner` is per clause), and `:storyboard` for the `RUN-FLOW` clauses, bound by the storyboard test and not by `:core`'s (§ T7.2) —, `:ui` for `SCREENS`;
...
- A table marked `data: NAME` with a `types:` line is extracted by the binder into
  ...
  except `GOLDEN_CELLS`, the cell table, emitted into `:core-testing`'s generated
  `commonMain` beside the golden fixtures, since the harness and the instruments both read
  it at run time and `:tools:instruments` may not depend on `:sim` (§ T2.1's edges; its
  owner stays `:sim`, § T7.2).
- **Granularity**: one clause per independently testable rule or per table; a clause the
  ...
  ≈ 230 rows, the skill table 24), and spec-lint counts prose lines only.
...
- generates `SpecTables.kt` and the golden hash constants into a generated `commonTest`
  source set, and `<Module>Fixtures.kt` and the cell table into a generated `commonMain` one — or the owning module's test source set where that module's ABI is dumped, `:engine`'s (§ T2.1) —,
  before compilation;
...
- fails the lane when a `contract` clause has no passing test in the lane's scope, when
  ...
  touched modules and whose binding tests carry a tag the lane runs — so not the `ART` and
  `RUN-FLOW` clauses, whose tests are L2b's, nor the `GOLDEN` and `BALANCE` clauses, whose
  replay and snapshot are L2a's; the commit lane's scope is every clause whose binding tests carry a tag it runs —
  everything but the clauses bound only by `Device`- or `Nightly`-tagged tests (the edge
  targets under gesture navigation, the forced tier drop, the device legs of the hash test,
  § V6's L4 rows), which the nightly's own binder run binds over its reports and publishes
  as a second matrix beside L3's, the one P5's gate reads for those clauses — and the binder
  binds once over `gate.sh commit`, reading L2a's and L2b's reports together — `ART` clauses bind only
  ...
  bound `proposed` clause. No lock file, nothing to rebase. The one exemption is the class
  the nightly binds: a clause whose only binding tests are `Device`- or `Nightly`-tagged
  merges `proposed` with its tests, the next nightly's matrix binds it, and its promotion is
  a later pull request of its own — the owner's contract-clause check applies there — which
  the L3 binder checks against the last nightly's matrix, read through the same
  `actions: read` the nightly status uses (`VERIFICATION.md` § V5), failing the promotion
  when that matrix does not bind the clause and never failing a `proposed` clause of that
  class for staying unpromoted.
...
of § T5.3's table, not the ≈ 230 cells — saying what the family fixes (the seeds, policies,
...
by id. **The schema**: any area file may carry a fenced block `schema: <Name>` — each screen file carries one, `schema: <Screen>State`; `spec/golden/fixtures.md` the `schema: GoldenFixture` block (first, the root: `leader: int`, `heroes: list<object:FixtureHero>` — each `id`, `awakened: bool` and `relics: list<object:VaultRelic>`, the wearer's own (every set bonus is wearer-only but BULWARK's battle-start shield, which the prototype grants to every living hero — `sets.ts`'s header, `battle.ts`'s opener) — `lap: int`, `pacts: list<string>`; no `hp` field: both readers set each hero's `hp` to `derive(member, {leader, pacts: []}).HP`, full health with no pact in the derivation, as the prototype's `freshFixtureParty` does, the row's `pacts` then entering the battle through `spawnPack`'s scaling and the battle-start openers as the harness passes them) and the `schema: FixtureHero` and `schema: VaultRelic` blocks it nests — `VaultRelic` the oracle's `Relic`: `id`, `slot`, `rarity`, `set`, `level`, `kindled`, `main: object:RelicMain` (`key`, `base`), `subs: list<object:RelicSub>` (`key`, `value`, `rolls`), `sigil` —, which `spec/fixtures/golden/fixtures/` uses; `spec/golden/vault.md` the `schema: VaultRelics` block, whose one field is `relics: list<object:VaultRelic>`, which `spec/fixtures/golden/vault/` uses; `spec/art/bible.md` the `schema: Ramps` block and `spec/art/seats.md` the `schema: Seats` block (§ T7.5) —
...
file of its own because a stem has one root type and `bible.md`'s is `Ramps`, § T7.4) — the shape
`{ground: string?, biomes: list<object:BiomeSeats>}` (`ground` optional, since the seat list lands before it) with `BiomeSeats` = `biome: string` (checked by spec-lint against the `BIOMES` `data:` table P2's fold writes from `prototype/game/data/enemies.ts`'s six biome names, § T7.6, each normalised to the `<BIOME>` token — runs of spaces and hyphens replaced by `_`, `EMBER_CRYPT`, the form `backdrops.ts`'s `backdropFor` already derives, since the prototype declares no ids and every name carries a space — the one spelling every CLI value, filename and `seats.json` entry uses; this file is read outside the binder, and the check is inert until that table exists, so from P0 to P2 the file carries the six tokens unchecked),
`seats: list<string>` (six actor ids, one per seat, in the anchors file's seat order `H0`, `H1`, `H2`, `E0`, `E1`, `E2` — three fallback heroes, three of the biome's pack, § T10.4) and `spread: string` — § T7.4's vocabulary
has no map type — one `ground` for the file and, per biome, six seat ids and `spread`, the
one id planted at all six anchors for the seat-spread reading; `ground` an sRGB hex, the
per-channel median over the surviving cells of all 72 strips of the six biomes' MED `flat=1`
resting frames, measured by `art rulers` at calibration — the seat list committed first, `ground` appended after
the calibration captures that read it; and beside it `spec/art/rulers-p0.json`,
`art rulers`' per-reading output over four frame sets — the LOW, MED and HIGH `flat=1`
frames and HIGH's diorama, four runs each stamped by `--set <key>` and merged into the one file under its key (§ V3.7) (per set: biome, seat, strip, the actor's and the strip's medians
and the ratio — 72 rows a set, 288 in all, under the keys `low`, `med`, `high-flat` and `high-diorama`), the three flat sets the input of the
equivalence checks (LOW and MED at P5, HIGH at P6, § T10.4) and the diorama rows P6's
diagnosis route, committed with the bars and owned as they are. `seats.json` is the one file
`capture.mjs seat=all`, `art rulers` and the Kotlin `frames --seats all` and
`frames --icon` all read (`art gate` takes the ground as `--ground <hex>` and reads no seat) (the prototype's one `seat=all|<id>` flag against the Kotlin's
`--seats all` / `--seat <id>` pair is deliberate — the frozen tool keeps its `key=value`
form, the Kotlin CLI its `--flag` form — and the two readings are the same two).

### T7.6 Reconciling and folding `DESIGN.md`

At P2, **before** the fold, the contract is reconciled with the code: the six items
`prototype/STATUS.md` names under "DESIGN.md — what looks wrong now that it is built" (the
pixelated-plane amendment and the unstated gain; the missing value-order law; the ship
criteria living in prototype/ART-REVIEW.md; the absent frame budget; `minAscensionFor` and the
...
`prototype/STATUS.md`'s three verifier-confirmed balance diagnoses left for the owner (`compare()`'s
...
line; every `schema:` block parsable, and every fixture under `spec/fixtures/art/**` — an area the binder never types, as `spec/fixtures/save/**`, which JVM tests read — validated against its root schema, and `seats.json`'s `biome` values are the `BIOMES` `data:` table's names as `<BIOME>` tokens once P2's fold has written it (§ T7.5; inert before); links resolve; a clause's prose ≤ 60 lines (tables exempt); no two clauses
...
(gitignored, rebuilt in every lane), which the art tool and `cells.mjs` read (P2's hand-run recording runs `gate.sh edit` first, so `spec/.generated/cells.json` exists before `cells.mjs` walks it) instead of
re-parsing the Markdown, so one parser reads every table; and no two cells of `cells.md` share a parameter tuple. Runs in the edit lane on
`spec/**`.
...
| actionlint, shellcheck, markdownlint, spec-lint; `html-validate` and `lychee` over `docs/**`, `lychee` over `plan/**` | workflows, scripts, documents, the spec, the two pages, the plan | L0; the two pages and `plan/**` in L3's lint job |
...
is a suppression without a reason. **One dated exemption**: `:core` carries a module-wide suspension of the complexity and length budgets from P1 to the P3 gate (§ T5.1; P1's synthetic module is generated under it), and inside it, during P3, functions of `:core`
...
A budget is raised only by a commit that says why in `CLAUDE.md`'s budget table; `:core`'s module-wide suspension of § T8.1, with the `port` budget inside it, is the only suspension and it has an end date, the P3 gate.
...
the same buffer on every platform — ARCADE builds the bright layer LOW alone skips, at LOW's cost plus the quarter-resolution pass, so its halation is the bright layer's glow and not the prototype's whole-frame snapshot, a change `FUNCTIONAL.md` § F1.3's table names —, so bloom and halation never both run; scanlines, the
...
| Reference iPhone 12 class | MED | frame p95 ≤ 12 ms; peak resident set ≤ 250 MB | L4 XCTest metrics (`iosApp/PerfTests`: a custom `XCTOSSignpostMetric` over the per-frame `os_signpost` intervals `:engine`'s iOS frame loop emits in debug and profile builds — a P5 deliverable, since XCTest's own hitch metrics read UIKit signposts a Skia canvas never posts — and `XCTMemoryMetric` for the resident set) |
...
brief (spec/art + the actor's row) → prompt pack → generate (key-pose candidates, then every frame of the finalists)
  → normalise (alpha clean-up — the raw candidate kept beside the normalised frames as `<pose>-<frame>.raw.png` for the halo metric, in the run directory `art generate` writes, gitignored: `art accept` measures the halo there and writes the reading and the raw's SHA-256 into the frame's sidecar, and neither the raw nor the run directory is committed —, crop, integer downscale to the cell, palette quantisation onto the element ramp, keyline check, cell alignment)
...
nothing the owner did not load. The tool's counter mirrors that: `art generate --key <id>` (an explicit argument validated against `tools/art/keys.json`) gives
every call an id, writes a row `{id, kind: bakeoff|cast|plane, key, provider, actor, images,
cost}` — `key` the argument's id, which the counter sums by; `cost` the adapter's reported charge, else `images` times the unit price `tools/art/keys.json` carries for the key, and 0 for a subscription key — as its own file `assets/spend/<id>.json` before it returns — one file per call, so pull
...
was accepted (a run with nothing to accept leaves its spend rows to ride the next pack's pull request; the counter sums `main`'s `assets/spend/`, the local ledger committed or not, and the rows of every open art pull request read through the App's API with an installation token `ci/mint-token.sh` mints on demand (a token lives one hour; the script caches it until five minutes before expiry and re-mints, § T13.3) — `art generate` refuses when the mint or the read fails; `art gate`, the lane step, reads no pull request and needs none — so parallel packs cannot each spend the headroom), `art gate`
...
the portrait key — the caps by key id in `tools/art/keys.json`, owner-reviewed as every
`tools/art/**` change is, the counter's one source; a key absent from it refuses every call) — so the stop trigger of
...
it is used — and re-read at P4's start and before any later character's generation
(`FUNCTIONAL.md` § F4.3) by the agent, the owner confirming the dated entry in the accepting pull
request, about an hour inside README's admin band (its P4 row), each re-reading dated in `LICENSES.md`, a changed term the same
tripwire as a retirement (README risks):
...
| Gemini image models | reference-conditioned generation and editing; strong consistency | per image, metered by Cloud billing, which alerts and never caps — its hard cap is the API's per-day request quota lowered in the Cloud console — a rate, so P0's terms check records that quota times the days to a revocation date the owner writes in the register and keeps — a Cloud API key carries no expiry field, so the revocation is the owner's act on that date, on the P0 owner row and the Monthly row — a fixed product below the key's cap, or admits Gemini on a prepaid balance only, or Gemini leaves the bake-off | portraits and the painted look; API terms to be confirmed; without a confirmed cap the painted arm is FLUX.2 alone |
...
sidecar, and adds, from prototype/ART-REVIEW.md's definitions, what the prototype never committed
...
under another seat's mask** (the seats of a rank stand 68 px apart on the ground rows and 46 px (pack) or 100 px (party) across, so a back enemy seat's strips, expected to fall under the next body at 46 px, would
...
taken over its masked cells — the cell at the median relative luminance, § F3.1's convention; `ground`'s per-channel median is a separate, colour-valued reading — and the mask comes from the producer too — each writes
`<frame>.masks/<seat>.png`, the drawn sprite's alpha at frame scale, beside the anchors
file. This ruler is re-derived here, not calibrated: prototype/ART-REVIEW.md measured its strips at
...
prototype/ART-REVIEW.md's **round-14** re-derivation: settle 20.4–38.8 %, idle 20.4–55.8 %, crown
...
definitions — a JVM test under `tools/instruments/src/jvmTest/kotlin/calibration/`, an owned path (`VERIFICATION.md` § V5), run in L2b beside the golden it checks, asserting
...
rule, over the fixed set of 36 seats and 72 strip readings, one per tier — the three-abreast anchors, which bound the prototype's two other seatings: a two-enemy pack's `ENEMY_FEET_PAIR` and a boss's `BOSS_FEET` lie on the same floor inside the three-abreast rank's span, so the derived pool and the ground strips under them are the ones the six seats read; P0 captures one pair frame and one boss frame per biome at MED as a check, its readings in the calibration report and not in `rulers-p0.json`, and a check reading outside the six seats' band is an owner decision recorded at P0, not a seat of the bar — LOW and MED over the flat composite through the `flat=1` path for P5,
...
`lineup.ts` measures them, and for look A the sheet's structural criteria at their
thresholds: a full 1-px keyline, the colour count (about 24, the element ramp plus
neutrals), cell alignment, one 8-connected component per non-dead bake, no halo, the silhouette
...
five § F3.2 names reported, not gating, for look B (prototype/ART-REVIEW.md's criterion 3, the chin shadow and the seams, is
the critic's). *Sheet, reported only*: contrast against the line-up ground —
prototype/ART-REVIEW.md's criterion 6 is **retired**. *Size, pass*: the bands of § F3.1 per actor
...
the six seats of each biome's resting frame (every actor on `idle` frame 0, § F3.1), 36 seats and 72 strip readings, the seat list
...
the app's bloom is bright-layer, § T9.4, and the ruler frames are rendered with it off,
`frames --bloom off`, so a prop glow or the sky body cannot move a reading; and P5's first
ruler run is an equivalence check — the Kotlin `frames` over the committed composite at
LOW and at MED, the `flat=1` frames of both tiers, against P0's 72 readings per tier as
`spec/art/rulers-p0.json` holds them (§ T7.5), reading by reading — a pass is every reading's ratio within 0.05 of P0's and none on the
other side of 1.5:1 from P0's, so the tier's count is P0's; the offsets recorded in the register and closed by the
rig, one rework of its constants inside P5, before either count bar is held (under README
question 2's portrait branch the check runs first, on the landscape composites at the
landscape anchors before `Layout` is re-cut, inside that question's priced session, and the
portrait bars are then re-recorded on the Kotlin frames as § F3.1 says); and at P6, once the Kotlin rig draws HIGH, the same check at HIGH over the committed
composite against the HIGH `flat=1` rows of `spec/art/rulers-p0.json`, before the HIGH bar
is held over the landed planes, with the same rework and re-record rule; still beyond
that after the rework, the bar is re-derived on the Kotlin frames — re-recorded at the
Kotlin count with the owner's approval in the same pull request, the offset staying in
the register as the two renderers' known difference), not over the
diorama; P6 at HIGH on the landed rig — after the HIGH equivalence check above — with LOW and MED re-read over the landed planes — which carry the NEAR painter P0's flat frames did not, P6 code authored to the bar — and not below their recorded bars (P0's, or a bar re-recorded under `FUNCTIONAL.md` § F3.1, upward at P6 where the count exceeds it); ARCADE
...
height) over the median of the six seats' torso medians of the same biome frame, reported as the maximum over the six
...
*Halo, pass*: measured on the raw candidate `normalise` keeps beside the frame (`<pose>-<frame>.raw.png`, § T10.1), before its alpha clean-up — the raw downscaled by the frame's own integer factor with nearest sampling, alpha preserved, so a cell here is the cell everywhere else — with the
silhouette defined as the union of the 8-connected components of at least eight cells at
α ≥ 128 (a dead frame's detached dagger or shatter is inside it; a stray pixel is not) — at
...
silhouette (a stray pixel, which the component definition leaves outside). *Portraits, pass* (the
...
alone need not), each cell assigned to its nearest ramp (a cell whose distance to an element colour is within ΔE 1 of the winning neutral's counts for the element, since element accents sit within ΔE 1 of neutrals the ramps do not share), the neutral ramps the element does not share —
...
changes. Under look B every measurement — the bake-off's, P4's gate's and L2b's asset gate's over the committed 128-px frames alike — is taken on the frame downsampled bilinear to its actor's cell canvas (64, or 96 for a boss), skipping the quantisation and the integer
downscale, so the
per-cell value, silhouette and motion metrics — the size bands, the part travel, the crown rise, the halo's component and ring — run on both looks at one resolution and mean the same thing.

### T10.6 Provenance, licensing and continuity

`assets/actors/<ID>/manifest.json` and `sheet.png` (the contact sheet the owner judged, excluded from the atlas and the install, § F3.5) — beside the fifteen frames, each with a
`<pose>-<frame>.json` sidecar of the fallback cast's shape (§ T10.9: canvas, cell, element, pose,
frame, feet, hitRect) plus `halo` — the reading and the raw candidate's SHA-256, an accepted-cast field the fallback frames lack, since they were captured and not generated; the raw is not committed and L2b's asset gate takes the halo from the sidecar —, written by `art accept` from the normalised frame — `hitRect` derived as the silhouette's full bounding box — its top the light box's, its width the HP hairline's, as the prototype uses its near-full-height `HERO_HIT_SIZE`; the 0.33–0.72 torso band is the ruler's median alone, § F3.1; the registered tap rect is that box clamped to the actor's own cell, the Voronoi split at the midpoint between neighbouring feet (`spriteCellX`, `rankFrontier` in DESIGN.md), so no stage point is owned by two actors — never authored as the prototype's per-recipe `hit`/`hitSize` are, and the fallback sidecars carry the same field computed by the same rule from the captured frame, so a mixed cast ships one hurtbox definition — which is what the
...
- **`assets/fallback/actors/<ID>/<pose>-<frame>.png`** — every actor's fifteen bakes at cell
  ...
  cast|dead, frame: 0..2, feet: {x, y} in cells from the canvas origin — `feet.y` the floor line, the row below the last opaque row, as the prototype's studies and recipes carry it —, hitRect: {x, y, w,
  h} in cells }` named `<pose>-<frame>.json` beside each PNG (an accepted actor's sidecar adds `halo`, § T10.6; a captured fallback frame has no raw candidate). Three uses: the gate's
...
- **A run** is `(RULES_VERSION, seed, RunConfig, [debug], decisions[])` in `:core`'s canonical
  ...
  snapshot plus the hero turns since — the open pending's drawn payload saved as drawn (SUMMON's offers, RELIC's cards, the SHRINE's pact, FORGE's rebrand lists, ALTAR's and REST's candidates: drawn before the pending is raised, so the post-draw rng word cannot reproduce them), the map's offers and the terminal fields (`won`, the death kind and killer) re-derived on re-entry — enumerated field by field in the `SAVE` clauses, so
  ...
  under the installed rules and the player is told once; an open battle restarts from its
  pre-battle snapshot (a replay of its recorded turns would index `actOptions` under rules that
  may have added a skill or moved a legality rule, so an in-range index could name a different
  act, which a range check cannot tell); an open room keeps its saved payload where
  the new rules still accept every item in it and otherwise re-enters at the node boundary
  and re-rolls it; the decision log restarts from that point
  ...
  together, never inside § T5.3's text encoding, which stays the log alone (§ F2.7's
  share appends the snapshot slot only when the log is a `resumed` one, so that share
  still replays, from the resume point; a never-resumed run's share is the log alone); the marker is a save-only line after the header, `resumed
...
- **`RULES_VERSION`** is an explicit constant in `:core`'s `version` package, bumped by the
  ...
  re-recording or a trace-format change. A corpus of saves under `spec/fixtures/save/v<N>/` (laid out by version, outside § T2.1's stem rule, since the `save` area is binder-excluded)
...
- **Accounts and testers (P0)**: the Apple Developer Program and the Google Play developer
  ...
  an Android pool is then the owner's choice, with a floor of three to five Android testers on the internal track, since that branch gives up every external Android reading — recorded in the register. A personal Play account created after November 2023 must run a
  ...
  sixteen on Android 10 or newer — 12 or newer preferred, each tester's version recorded at recruitment, since D11's fallback at P5 drops any tester below 12 — the list and the weekly play record are the owner's own personal-data holding: store-account emails and versions alone, said so in the recruitment message, deleted when the closed test closes — for Play (four over the rule's twelve, the target and not the gate — the fourteen days start at twelve or more opted in — the surplus of the pool a reserve that opts in before a dropout takes the count to twelve, since a dip resets the clock), at least three on iPhone with iOS 16 or newer, versions recorded at recruitment, for
  ...
  one to four weeks before P5's first build by pace, the closed-track and external-TestFlight invitations following the
  last stop (three to five internal-track invitations precede it by three to five days).
- **Store set-up (P5, before the first device build ships to testers)**: the App Store name,
  ...
  (38 weeks at one session a week, 53 with the contingent items at their top, longer below it), and if any limit shorter than fifty-three weeks
  exists only the bundle id is chosen at P0 and the name waits for P5, a lapsed record taking the register's fallback name), the application id chosen
  ...
  with its copy and graphics — screenshots from the instruments' `shot --store <class>` — the logical frame at the canvas's own `k = h/720` (1.833 on the 6.9″ canvas — nearest
  where `2k` is integral, bilinear otherwise, § T9.1), as the device draws it — a store
  screenshot is not a golden —, letterboxed with the recorded ground colour (`seats.json`'s `ground`) into the store's canvas, since App Store
  Connect validates dimensions at upload — the build declares `TARGETED_DEVICE_FAMILY = 1`, iPhone only, so no iPad class is asked for (`FUNCTIONAL.md` § F1.3): 2868 × 1320 for the 6.9″ class and 2688 × 1242
  for the 6.5″ class, landscape; Play takes the 1280 × 720 frame itself (16:9, 320–3840 px)
  — and a 1024 × 500 feature graphic (all inside the store set-up's S) — which Play needs complete before a closed-track
  release, App Store
  ...
  TestFlight's **internal** group — the owner's own install path, and all the first tag does (three to five of the lined-up Android testers join Play's internal track on that first build, three to five days before the last scheduled stop, which is taken after they have played — before the store set-up, README's P5 row; TestFlight's internal group is App Store Connect team members alone, so iOS's first non-owner reading is the external group's closed-test build),
  since it lands before the store set-up; the internal tracks' own prerequisites (at
  writing the record and its ids, Play App Signing's enrolment and Apple's
  export-compliance answer — Play Console Help at writing lets an internal test be set up before the store
  listing, the content rating and the data-safety form are complete, so the App content
  declarations gate the closed track and production) are confirmed at P0's terms check, and any found to gate an upload moves
  ahead of that tag — and, from the first build after the store
  ...
  App Review of about a day on the first build, and is an S inside P5; its builds expire at
  90 days, so an external test that outlives one gets a refresh build — a tag, a `release`
  approval, a Beta App Review, an S and one owner act — under either branch of README question 4(a)); version name from the tag,
  ...
  statement of that trigger; the fourteen days start when twelve or more testers have opted in,
  about a week after that build, sixteen the target the reserve is still invited toward, and Play's first review of that release takes days.
- **The stores (P7)**: the owner submits the iOS build for **App Review** (typical
  ...
  and after the fourteen days applies for Play **production access** — the form's written
  answers on recruitment, feedback and changes drawn from the triage record — and passes Google's
  review, which checks that the testers used the app; every production release goes out
  as a staged rollout on Play (a tenth first, widened after a week without a new crash
  cluster) and, for updates, a phased release on the App Store (Apple offers it on automatic
  updates alone, so v1.0 — the release most likely to carry a run-breaking defect — has no
  phase on iOS: its halt is Remove from Sale plus an expedited-review fix), so a bad release
  can be halted before it reaches everyone — halting it is the steady state's first act on
  a run-breaking report (README); the P7 checklist carries both, with
...
- **Release notes** are generated by `release.yml` from the clause ids in the pull requests merged since the last tag (read through its `pull-requests: read`) and written into the stores' what's-new fields through the `release` environment's App Store Connect key and Play service account; no GitHub Release is created, so the workflow needs no `contents: write`.
...
- `SessionStart`: run `ci/env/setup.sh` where the host is not the image (idempotent,
  ...
  no-op), install `ci/mint-token.sh` as git's credential helper and confirm one mint — an installation token lives one hour and a session twelve to sixteen, so the helper and `art generate` (§ T10.1) mint per operation from the App's private key, each token cached until five minutes before its expiry and re-minted after.
...
against a tree another agent is editing — the lesson `prototype/STATUS.md` records.
...
private device-runner repository appends its rows as comments to the standing `ledger` issue in this repository, created at M2, which its token's `issues: write` allows and the nightly reads (or the farm's rows
...
the committed `kmp-*.yml` or `ci/required-checks.txt` differ from what the head's generator writes (a list that omits a job is caught here, since the rules read fails only on a listed job); the hand-written
...
hand-written jobs that `ci/lanes.yaml` carries under `required-hand-written:`, which the generator concatenates (`prototype.yml`'s three — `prototype-freeze`, `prototype-check` and `prototype-changes` — and `env-image`'s two, `env-image-changes` and `env-image`, prefixed like the generated ones) and the three gate-App checks from `lanes.yaml`'s `required-gate-app:` table, the one spelling `owner-review.yml` and the ruleset copy —
and an L3 job reads the branch's active rules through the API — `GET /repos/{owner}/{repo}/rules/branches/{branch}`, which asks no more than the `metadata: read` every `GITHUB_TOKEN` carries implicitly, on the private branch of README question 8(b) too — and fails when a listed
job is not required (it passes while the branch's rules carry no required-checks rule — M2's provisional ruleset has none — and bites from M4, whose ruleset carries the list: the M3–M4 window § V1 names beside M2's `CODEOWNERS` one); adding one is the owner's act, named in every phase that adds a job
(P1's M7 — the Android and iOS compile jobs, one owner ruleset act inside P1's eight sittings; the wasm job is informative until P8 — 5b's arm64 leg at M4, generated with the merge lane's jobs when the spike passed and inside that act; P3's `diff-oracle`, golden-replay and emulator hash jobs — `:sim`'s two, delivered at P3, and the x86-64 emulator with KVM the gate needs, which M7's compile jobs do not carry — one act on the P3 owner row (P2 adds no lane job: its goldens are frozen data under `prototype-check`'s freeze and the spec lint); P5's L3 additions — the emulator and simulator boot smoke — (the device lane is L4's and posts no required check); P8's wasm) — the pull request that adds the job fails
...
request is open (at M4 the art pull requests held open since P0's exit merge first — and before P3's act P4's open pack pull requests are merged the same way, since P4 runs beside it —, then
the list is added, the bootstrap's own throwaway pull request exempt from the rule); removing one runs the other way, the owner dropping the rule first and the
job leaving the workflow after — scheduled as an addition is, the rule dropped while the removing pull request is open and no other owner-gated one is, since `ci/required-checks.txt` names the job until that pull request merges and the list job would fail every other open pull request meanwhile — because a new required check reads "expected" on an older head until a
...
builds never included in the root `settings.gradle.kts` (`plan/spikes/5b/` for spike 5b) —
except spike 6's `setup.sh` and `env.Dockerfile`, created in `ci/env/` in place (§ T4.1; `ci/mint-token.sh` is the move commit's, since that commit is the first push),
an owned path M1 reviews there, its report still under `plan/spikes/6/`. The agents' App (README question 7(a), or its fallback) is created before any agent commit — the third of P0's acts, after this check and questions 8 and 3(c) (README, the P0 owner row) — since no agent pushes without it; the move commit of § T4.1 lands
...
|---|---|---|---|
| **P0 Decide, spike, freeze, bake-off** (XL) | this plan approved | the owner's answers to questions 1, 2, 3(b), 3(c), 6, 7, 8 and 10, and the character brief of `FUNCTIONAL.md` § F4 or the acceptance of § F4.3's price for a later change; the terms check — the owner's, about four to six hours inside the 35–65-hour admin band, the Play account's creation date, the subscription tier and question 8's GitHub rates and private-plan protection rules its first hour before the go and the rest after it, the name clearance two of them: the Play account's creation date, Play's inactive-account policy and both stores' current forms, the internal tracks' prerequisites, the App Store name's limit, both trader declarations, the three GitHub rates and the environment protection rules a private repository's plan admits, the agents' subscription's terms of use against the pattern the plan runs on it (unattended overnight sessions, several agents in parallel), each provider's terms and cap mechanism and the subscription provider's plan, price and allowance, the agents' subscription tier, its limits and whether a higher tier or a second seat is available (the calendar's first term rests on them, README), the name clearance and the territories — recorded in the register and `assets/LICENSES.md`; the Apple and Google accounts opened and the twenty to twenty-five testers' count and platform mix decided (they are lined up at P2's exit); the GitHub App created and installed; **the owner plays the prototype on a phone as the owner's first act after the move commit and question 3(c)'s fix, before the accounts, the testers and the bake-off's spend, records `plan/BASELINE.md` and says go, no-go, or go with a named mechanic to change on the game** — the third answer sized at `FUNCTIONAL.md` § F4.3's rules row, built after P3 under § T7.7, before or after P5's first build as the owner decides — recorded in the register; the move to `prototype/`, the banners, the Pages workflow, the stub `CLAUDE.md`; the captures of § T4.2 (the fallback sheets, the flat backdrops with light data, the study, the sounds, the glyph tables); the `PixelActor` registry; **the art tool** with the full metric set, calibrated, the bands recorded; provider terms verified into `LICENSES.md`; the option-B rate measurement — EMBER's remaining fourteen frames drawn by hand, half a session, an S outside the aggregate, run only if the owner is weighing a "no" on README question 6 and before that question closes (the money table's *Option B* row); spikes: (1) headless JVM capture of a Compose stage to PNG at k = 1 and its time, on `agent-env`, and the same stage — the two blend modes, one lit frame — on the iOS simulator through spike 5's hosted-macOS workflow or the owner's Mac, since iOS is half the product and nothing else renders it before P5; (2) `RunSession` on intrinsics with a toy generator, `HERO_TURN` at step 7, `ENEMY_TURN`, and the three paths of the self-check with draw burning; (3) Pitest on a KMP JVM target; (4) detekt 1.x and 2.0 on Kotlin 2.4 syntax with type resolution timed on a KMP tree; (5) Kotest 6 running one hash test on the iOS simulator (a throwaway hosted-macOS workflow or the owner's Mac), as an Android device test on an x86-64 emulator with KVM, and on wasmJs; (5b) the same hash test on a free `ubuntu-24.04-arm` runner — the JVM, and an arm64 Android system image if it boots — which decides whether the arm64 arithmetic truth lives in L3; spike 1 also renders the stage's `ColorDodge` and `Multiply` on the Android emulator at API 28 and 29 — the fact D3 rests on — on a throwaway hosted-Linux workflow with KVM (§ T1); (6) the environment: `setup.sh` on `agent-env` with its cold start, the image built once from `env.Dockerfile` to its registry by a throwaway workflow with `packages: write`, tagged by its commit as `env-image.yml` will tag (its digest is pinned at P1, `VERIFICATION.md` § V9), and an agent session minting App tokens on demand through `ci/mint-token.sh` and opening a pull request, one mint forced past the hour; (7) the sprite bake-off on six actors, options (a) and (b), both looks, under § T10.7's protocol, through the calibrated gate, shown in lit phone frames from the prototype's stage; (8) the pinned critic scoring the prototype's round-14 calibration sheet — the full-frame set the five axes are scored on: the cast's lit frames, the HUD, a hit peak and one resting stage frame per biome, as prototype/ART-REVIEW.md's round-4 `8 · 8 · 8 · 8 · 7` was scored — three times under the protocol, the sheet committed under `spec/art/calibration-sheet/`, the medians recorded **per axis** as the **baseline** every later bar is set against; `tools/art/keys.json`'s rows for the bake-off keys (an owned path, so each key's row is an owner-reviewed pull request before that key's first call — the cast keys' rows at P0's exit, P6's plane key's at P5), the ramps export and the stage fixture page of § T4.2 — in P0's post-go order: the captures, the ramps export and the stage page, the sheet calibration, the seat list, the ruler captures, the `ground` reading and the three bars, then the bake-off | spike reports with frames, numbers and a recommendation each; the owner's go, no-go, or go with a named mechanic to change recorded; P0's re-estimate accepted by the owner, or the programme stopped, before P1 starts; the calibration's disagreements adjudicated; the look fork closed; the three in-scene bars recorded, each at or above 60 of 72 or the owner's decision recorded (`FUNCTIONAL.md` § F3.1); the critic's per-axis baselines committed to `spec/art/` beside the protocol and the calibration sheet; the decisions recorded in the register; the sizes re-estimated |
| **P1 The rig** (L) | P0 | the environment image (built at spike 6) rebuilt by its own workflow with its digest pinned in `lanes.yaml`, and `setup.sh` proven equivalent by `env-check` on `agent-env`, macOS (the owner's Mac or a hosted-macOS job) and L3's bare runner; the calibration golden's second reader (§ T10.4); the gate App and its `gates` environment, and Renovate installed; the owner-review workflow and its re-dispatch; the provisional ruleset and `CODEOWNERS`, the two throwaway pull requests that prove the review mechanics, the required checks, the tag ruleset, the `release` environment, the prototype workflow (the freeze check and `prototype-check`, required) and, if question 5 chose the runner, the private device-runner repository; every module present and empty with the convention plugins, every analyser at full strength, the spec binder and its tests, `lanes.yaml`, `gate.sh` and the generated workflows, the hooks, `CLAUDE.md`, the `kmp-quality` rubric and the other skills; the hello-world stage (one fallback sprite over one flat backdrop, one button, one screen test, one golden recorded in the image and compared on the owner's Mac, one storyboard step in both modes); **a synthetic `:core`-sized module** generated from the prototype rules' function-size histogram (5 000 lines, 300 tests with property tests) and **a synthetic storyboard** that the lane budgets are measured on | every lane green (for L3 and L4 the P1 subset `VERIFICATION.md` § V9 names) and inside its P1-measurable budget on the synthetic module, timings recorded in the bundles' `lanes.json` with machine classes; the sizes and the calendar re-derived and **accepted by the owner, or the programme stops here** (the cast's generation paused while the decision is pending, as it is while a store enrolment is refused or suspended, the art spend committed so far stated) |
| **P2 The spec and the oracle** (L) | P1 | `--trace`, `--cell <id>`, `--ascension`, `--path`, the canonical `--dump`, the strong-party and stall fixtures, `coverage.mjs`, `cells.mjs`, `fixtures.mjs` (the fixtures' run-time reader), the fixtures export and the battles-mode knobs and `strong=1` in the prototype's harness and driver; the Node pin confirmed and `engines` added; tag `ts-oracle-v3`; the prototype-freeze check; the golden set recorded on the pinned Node under `cells.md` and hashed, with the coverage report; the contract reconciled with the code, then folded into `spec/` clauses (status `proposed`), the `known-divergence` notes of `FUNCTIONAL.md` § F1.4 among them; the clause count per area | spec-lint clean; every clause has an id, an owner, `paths` and a status; goldens frozen; every event, status, set, sigil, pending kind, room type, ascension row, pact, forge mode and skill id appears in the golden set (but the kinds § T5.3 exempts; a cell is added until it does); P3 and P5 re-sized against the clause count and **accepted by the owner, or the programme stops here** (generation paused while the decision is pending, the committed art spend stated) |
| **P3 The rules** (L) | P2 | `:core` test-first in § T5.1's order; `:sim` with every command; the `BALANCE` clauses promoted to `contract` with the rules clauses, bound by the reduced snapshot; `:core-testing`; the save format and the `SAVE` clauses designed around question 10's answer; the cross-platform hash test; the ABI dumps; the port exemption zeroed at the end | `diff-oracle` clean on the golden set; the balance tables reproduced; the hash test green on the JVM, the x86-64 Android emulator and the iOS simulator, and once on an arm64 phone or farm device; matrix 100 % (no weak clauses) for the rules areas — the clauses whose `owner` is `:core`; coverage and mutation at threshold or the fallback recorded |
| **P4 The cast** (L, parallel with P1–P3, from P0's exit) | P0's bake-off | the pipeline at full strength; the cast generated in `FUNCTIONAL.md` § F3.5's order with the stop decision after the six heroes, judged on the prototype's stage frames; portraits; every accepted actor committed with its manifest under the owner's review | every actor passes the gate; the six heroes meet the value targets the rejected kit fails (re-derived on look B's reference if question 1 chooses painted); the full-frame critic's sprite axis at least one above P0's recorded baseline under the same pinned protocol, capped at 9 (expected 9 against 8; the old 8 was a single unprotocolled verdict and proves nothing; a baseline already at 9 leaves the cap nothing to give, so that axis's bar is an owner decision recorded in the register before P4 starts); the owner accepts on a phone |
| **P5 The stage and the screens** (XL) | P3 (+ P4's heroes for the felt rows — or, when P4 is late, the fallback heroes as a mixed cast the owner approves by name, the art acceptance deferred to P6) | `Frame`, `Layout`, `Focusables`, `Stage` and `StageScene`, the light rig from the light data over the flat backdrops, atlases, VFX, pops, audio; the two tiers and ARCADE (HIGH with P6's planes, `FUNCTIONAL.md` § F1.3); the bloom decision (§ T9.4; before the closed test's build, since its fallback would reset a running closed test); every screen as Compose UI over the seam, designed for the phone; `FUNCTIONAL.md` § F2.2–F2.7 (the bug-report export with the first signed build the owner installs) and, under question 10's yes, F2.1's resume path; fixtures for every screen state; semantics tests; goldens; the storyboard driver with the strong-party fixture and the forcing hooks (set act and lap, jump to a room type, force a pack, set a hero's hp — `spec/platform/` clauses; `RunDebug` in `:core` a post-gate change under § T7.7, § T2.3); the phase's new required checks added by the owner when the L3 list check names them; the Android and iOS shells; the `:app:benchmark` Macrobenchmark module (`com.android.test`, § T2.1) the device lane's benchmark APK comes from; the `PerfTests` XCUITest target under `iosApp/` for the iPhone's budgets (§ T9.5); device boot smoke; the store set-up and the signed build pipeline — `release.yml` on a `v*` tag, hand-written (§ T12, § T2.1) with the privacy page, the testers' channel and the stores' own crash signal; the owner's last scheduled stop, on the first playable build — a signed build on the internal tracks, so the pipeline and the first tag precede it; the closed track and the external group start after the store set-up — and before the listings, the forms, the invitations and the closed test (README, the P5 owner row); the closed test started on Play's closed-testing track and on external TestFlight with testers from the same pool, on the build the README's absence protocol names, and its reports triaged (an S inside the XL); a first build over the fallback actors where the cast is not yet accepted is a mixed cast the owner approves by name; L2b's content-scaled parts measured; the memory and size table re-derived; the cast re-judged on the real stage | storyboard `PLAYFULL OK` — every biome, boss and screen reached — and a two-act KO run on the JVM in skip-playback mode, the full-playback storyboard in L3, two acts on Android and iOS; every fixture has a golden; `screens` and `platform` clauses bound (those bound only by `Device`- or `Nightly`-tagged tests in the nightly's matrix, § T7.2); the desktop and reference-phone budgets met; after § T10.4's equivalence check (the Kotlin rig at LOW and MED against P0's 72 readings per tier, every ratio within 0.05 and none across the 1.5:1 line, the offsets recorded; beyond that after one rework, the bar re-recorded on the Kotlin frames with approval, § T10.4), the in-scene rulers at their P0-recorded bars for LOW and for the reference phone's default tier (MED) over each biome's resting frame (under question 2's portrait branch, reported and owner-approved as the portrait bars that P6 then holds, `FUNCTIONAL.md` § F3.1) — the fixed set of 36 seats and 72 strip readings, the fallback cast as the reference (`art rulers` in L2b over `frames --cast fallback --seats all --bloom off`, and over `--seat <spread id>` per biome for the reported spread; and once over the accepted cast at P5's end, reported — the party's and the enemy rank's medians, the regression scene round 6 fixed, which the fallback-cast gate cannot see; a count below that tier's recorded bar takes the cast-miss branch, a seat the bar concedes conceded for the cast too); the full-frame critic's UI and VFX axes at least one above P0's per-axis baseline, capped at 9 (a baseline already at 9 is an owner decision) — a miss on either has the cast-miss shape: one rework of the screens or the effects inside P5, then the owner's decision, a recorded miss with the bar re-recorded in the same owner-reviewed pull request, or the phase held —, with all five axes recorded at P5's end, and the sprite axis not below its P4 reading (not asserted where the frame carries fallback heroes under the mixed-cast approval: the sprite axis is then recorded only, P6 takes both it and P4's deferred acceptance, and P0's baseline is the floor until P4 reads) — a miss is the rig's to fix inside P5 (the gain, the pools and the cast lobe are P5 code), never the sprites', with the cast-miss bound: after one rig rework still below, the owner accepts it as a recorded miss carried into P6 — the bar re-recorded in the same owner-reviewed pull request, `FUNCTIONAL.md` § F3.1 — or holds P7 for P6 (question 4(a)); the felt rows signed by the owner on a device (one Android at or below the reference class among them, § T1), or accepted as recorded misses under `FUNCTIONAL.md` § F1.5's bound |
| **P6 The scene** (L) | P5 (+ P4) | the six biomes' planes as data-driven painters — the default — or, if README question 4 approved them by name with their budget line, AI-generated planes judged by the scene rulers, the critic and the owner (`FUNCTIONAL.md` § F3.7); the light wells, hues, bright-mass and plate-rule items of `prototype/STATUS.md`'s round-5 brief; the flat placeholders retired | after § T10.4's equivalence check at HIGH (the Kotlin rig over the P0 composite against the HIGH `flat=1` rows, the rework and re-record rule as at P5), the in-scene rulers at their P0-recorded HIGH-tier bars over each biome's resting frame with the fallback cast as the reference, LOW and MED re-read over the landed planes (the NEAR painter P0's flat frames lacked is P6 code authored to the bar) and not below their recorded bars (P0's, or a bar re-recorded under `FUNCTIONAL.md` § F3.1, upward where P6's count exceeds it); the full-frame critic's scene and composition axes at least one above P0's per-axis baseline under the protocol, capped at 9 — a miss on either has the cast-miss shape: one rework of the planes inside P6, then the owner's decision, a recorded miss or P7 held (README question 4(a)) — (the rejected look scored 8 · 8 · 8 · 8 · 7 — an absolute 8 already met on the scene axis, not on composition, which stands at 7; a baseline already at 9 makes that axis's bar an owner decision recorded before the phase), and the sprite, UI and VFX axes not below their P4 and P5 readings (P6 also takes P4's deferred acceptance of late heroes, `FUNCTIONAL.md` § F3.5); the owner accepts on a phone — a ruler below its bar or a no on the phone is one rework of the planes, then the owner's decision, a recorded miss or the phase held (§ F3.1) |
| **P7 Ship** (M) | P5 (+ P6 per README question 4) | credits and disclosure; the store identity re-cut over the accepted cast and the listing update it forces, an S (`FUNCTIONAL.md` § F2.5; in P6's release line where hero acceptance was deferred under question 4(a)'s recommended branch); the closed test's feedback triaged and answered, and read once as a product signal at P7's start — the owner's ship, fix-first or re-plan decision, the scene- and cast-class reports routed to P6 and P4 and excluded from it, recorded in the register (README, the P7 owner row) — a report that breaks a run (a crash, a stuck screen, an unexpected lost run — under question 10's no, a run lost to the platform killing the app is the designed loss, a recorded known miss, not a blocker) is a blocker that holds the `v*` tag and is fixed inside the M whatever it touches, a rules fix under § T7.7 with its clause — the first inside the M, the second and the third an S each, beyond which P7 re-sizes or the owner decides; other presentation fixes inside the M; a triage list of the remaining rule-touching reports the owner decides as § F2 or § T7.7 changes after P7 (an S–M in the first steady-state session); the App Store submission and App Review; the Play production-access application after the closed test's fourteen days — the stores' clock, about five weeks from the closed test's build, is the calendar's third term (README) | installed from both test tracks; the first-ten-minutes test passes on the owner's phones; App Review passed; production access granted — or, under the tester-shortfall fallback, Android held in closed testing as a recorded state while iOS ships |
...
| A developer account that cannot be opened, or whose verification is delayed or refused | both enrolments started right after the go; the name reservation blocked until Apple's membership is active, the fallback name ready; a refusal by one store ships the other alone; by both, the programme stops at P1; either way P4's generation pauses until that decision is taken (README risks) | an enrolment unresolved fourteen days after submission |
...
| The iPhone lane's development identity leaks from the runner host | a development-only `match` branch (§ T1), never the distribution material; revoked and re-issued, the lane on the farm meanwhile | a certificate the owner did not issue; a failed device build |
| The runner branch's accepted risk: unreviewed `main` executing nightly on the owner's Mac (README question 5, D18) | the Android lane in a Linux VM with the phone and the runner token alone; the iPhone lane under its own user account; the farm the alternative | anything on the host the nightly did not write |
