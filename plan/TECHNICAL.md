...
| `:tools:instruments` | `:core`, `:core-testing`, `:ui`, `:engine`, JVM libraries | — |
...
| The stage anchors | `HERO_FEET` and `ENEMY_FEET` exported with `rig.json` (§ T4.2) and bound as `spec/platform/` constants — the landscape stage's six seats, which the pools derive from and the in-scene bars are read at |
...
- the `flat=1 tier=<LOW|MED>` frames, for the LOW and MED bars: the biome's unlit composite
  ...
  36 seats and their masks —, and one small engine hook — a flat plane supplied as an image: at LOW, `bakeFlat`'s key
...
- `prototype/tools/capture.mjs backdrops flat=1`: an `unlit` flag on `engine/light.ts`'s
  **bake** path (`bakeFlat`, which at LOW paints the key light and the grade's multiply
  ...
  `anchors: {hero: HERO_FEET, enemy: ENEMY_FEET}` from `layout.ts` — bound as `spec/platform/`
...
- optionally, under question 3(c): the three battle-screen defects (two root causes) fixed in
  ...
  event); each run at `seed=N` before its fix, where it must fail, and after, where it must
  pass, the seed making both the same run — the headless `sim` cannot
...
party, the relics, the lap, the pacts and each slot's `awakened` flag** — so the lap and the pacts come from the row alone — there are no `--lap` or `--pacts` knobs — and the `battle-fixtures` literal means lap 1 and no pacts — a `party` value names a row of
...
tests bind them (§ T2.3). The fixtures — party, relics, lap, pacts, `awakened: list<bool>` per party slot (the route to the six awakened variants, § T5.3's coverage) — live in `spec/fixtures/golden/fixtures/*.json` (§ T4.2),
...
- `owner` names the module whose tests bind it and whose generated `commonTest` receives
  ...
  (§ T7.8) and whose tests report as JUnit XML the binder reads like any other task's; the owner per area, which is also the module whose `<Module>Fixtures` receives the area's fixtures (§ T2.1): `:core` for the rules areas, `RNG`, `SESSION`, `CODEC`, `SAVE` and `SYNTHETIC`, and for `spec/golden/fixtures.md` and `spec/golden/vault.md` — rules types, generated into `:core-testing` rather than `:core`'s shipped `commonMain` and ABI, read by `:sim`'s harness and by `:tools:instruments`' storyboard through the `:core-testing` edge, since neither may depend on `:sim` and the app ships no fixture — `:sim` for `spec/golden/cells.md` and `BALANCE`, `:engine` for `PLATFORM` — but `:core` for the `PLATFORM-DEBUG` clauses, which bind `RunDebug` in `:core` (`owner` is per clause) —, `:ui` for `SCREENS`;
...
`art rulers`' per-reading output over the LOW and MED bar frames (per tier: biome, seat,
strip, the actor's and the strip's medians and the ratio — 72 rows a tier), the input of
P5's equivalence check (§ T10.4), committed with the bars and owned as they are. `seats.json` is the one file
...
cost}` — `key` the argument's id, which the counter sums by — as its own file `assets/spend/<id>.json` before it returns — one file per call, so pull
...
portrait bars are then re-recorded on the Kotlin frames as § F3.1 says); still beyond
...
diorama; P6 at HIGH on the landed rig, with LOW and MED re-read over the landed planes — which carry the NEAR painter P0's flat frames did not, P6 code authored to the bar — and not below their recorded bars (P0's, or a bar re-recorded under `FUNCTIONAL.md` § F3.1, upward at P6 where the count exceeds it); ARCADE
...
`assets/actors/<ID>/manifest.json` — beside the fifteen frames, each with a
...
- **Store set-up (P5, before the first device build ships to testers)**: the App Store name,
  ...
  with its copy and graphics — screenshots from the instruments' `shot --store <class>` — the logical frame at an integer
  scale, letterboxed with the recorded ground colour (`seats.json`'s `ground`) into the store's canvas, since App Store
...
an owned path M1 reviews there, its report still under `plan/spikes/6/`. The agents' App (README question 7(a), or its fallback) is created before anything, since no agent pushes without it; the move commit of § T4.1 lands
