...
material, created at M7 beside the USB proof — the
...
- **Every workflow runs on every pull request; jobs skip themselves.** A required check
  ...
  (`ci/env/**`), `docs` (`docs/**` — ORed by the lint job, whose HTML and link check over the two pages is their one pre-merge reading, § T8), one per Gradle module — `core` (with `core-testing/**`), `engine`, `ui`, `sim`,
...
mode, hash) under a `GOLDEN-NN` clause, generated into `commonTest` as every `data:` table
...
kindled relic with it worn; an ascension row when a `cell` line carries it; a skill when a `CAST` line names it — the six awakened variants reach the set only through an ALTAR awakening, so an awakened-party battles fixture is the cheapest cell. Three kinds never
...
| Storyboard | `:ui`'s test source sets (the driver), `:tools:instruments` (the JVM CLI over it) | skip-playback: every biome, boss and screen reached with the strong party and the forcing hooks, once by tag and once by keyboard; a two-act run to a KO (L2b); full playback with frames (L3); Android and iOS, two acts (L4) | L2b · L3 · L4 |
...
`{ground: <hex>, biomes: list<object:BiomeSeats>}` with `BiomeSeats` = `biome: string` (checked by spec-lint against the `BIOMES` `data:` table P2's fold writes from `prototype/game/data/enemies.ts`'s six ids, § T7.6 — since this file is read outside the binder; the check is inert until that table exists, so from P0 to P2 the file carries the prototype's six ids unchecked),
`seats: list<string>` (six ids in anchor order) and `spread: string` — § T7.4's vocabulary
...
line; every `schema:` block parsable, and every fixture under `spec/fixtures/art/**` — an area the binder never types, as `spec/fixtures/saves/**`, which JVM tests read — validated against its root schema, and `seats.json`'s `biome` values are ids of the `BIOMES` `data:` table once P2's fold has written it (§ T7.5; inert before); links resolve; a clause's prose ≤ 60 lines (tables exempt); no two clauses
