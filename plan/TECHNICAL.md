...
`RunHost` in `:sim` that reproduces `:ui`'s token protocol — a host-protocol check on (b)'s
...
- **Every workflow runs on every pull request; jobs skip themselves.** A required check
  ...
  and an `if:` that ORs its own module booleans with `build`, `root`, `spec` and `assets`, so a
  ...
  (Renovate's batch is a `build`-only diff) — a job skipped by its
...
- fails the lane when a `contract` clause has no passing test in the lane's scope, when
  ...
  replay and snapshot are L2a's; the commit lane's scope is everything, and the binder binds once over
...
`{ground: <hex>, biomes: list<object:BiomeSeats>}` with `BiomeSeats` = `biome: enum:BiomeId`,
...
re-parsing the Markdown, so one parser reads every table. Runs in the edit lane on
`spec/**`.
...
MED against P0's 72 readings, reading by reading, the renderer offset recorded in the
register and closed by the rig before the count bar is held), not over the
diorama; P6 at HIGH on the landed rig, with LOW and MED not below their recorded bars (P0's, or a bar re-recorded under `FUNCTIONAL.md` § F3.1); ARCADE
...
silhouette defined as the largest 8-connected component of cells at α ≥ 128 — at
