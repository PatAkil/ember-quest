...
GitHub Actions to create and approve pull requests" off — the two settings that cap any
workflow copy a head branch carries, `owner-review-redispatch.yml`'s included
...
set bonus, sigil effect, pending kind, room type, ascension row and `SkillId` that **appears** in
...
- **The stores (P7)**: the owner submits the iOS build for **App Review** (typical
  ...
  cluster) and a phased release on the App Store, so a bad release can be halted before it
  reaches everyone — halting it is the steady state's first act on a run-breaking report
  (README); the P7 checklist carries both, with
