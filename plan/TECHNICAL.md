...
`{ground: <hex>, biomes: {<BIOME>: {seats: [six ids in anchor order], spread: <id>}}}`, one
`ground` for the file and, per biome, six seat ids and `spread`, the one id planted at all
six anchors for the seat-spread reading; `ground` an sRGB hex, the per-channel
...
neutrals), cell alignment, one connected component per bake, no halo, the silhouette
...
*Halo, pass*: measured on the candidate before `normalise`'s alpha clean-up, with the
