...
- `prototype/tools/capture.mjs backdrops flat=1`: an `unlit` flag on `engine/light.ts`'s
  **bake** path (`bakeFlat`, which at LOW paints the key light and the grade's multiply
  ...
  enabled with P6's planes and its bar (`FUNCTIONAL.md` § F2.6), the `flat=1 tier=HIGH` set serving P6's
...
If the rebuild is abandoned at any phase, nothing has to be undone: the prototype's demo builds and
