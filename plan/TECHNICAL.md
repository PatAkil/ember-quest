...
| actionlint, shellcheck, markdownlint, a custom spec-lint; `html-validate` and `lychee` (offline, the internal links) over `docs/**` | current | workflows, scripts, documents, the spec, the privacy and support pages | L0 |
...
| actionlint, shellcheck, markdownlint, spec-lint; `html-validate` and `lychee` over `docs/**` | workflows, scripts, documents, the spec, the two pages | L0 |
...
- **Accounts and testers (P0)**: the Apple Developer Program and the Google Play developer
  ...
  sixteen on Android 10 or newer — Android 12 or newer if D11's fallback is taken — for Play (four over the rule's twelve, the target and not the gate — the fourteen days start at twelve or more opted in — the surplus of the pool a reserve that opts in before a dropout takes the count to twelve, since a dip resets the clock), at least three on iPhone with iOS 16 or newer, versions recorded at recruitment, for
