...
| `HERO_TURN` | `actor=H<slot> options=<skill>:<target>,…` — `<skill>` the `ActOption` skill index into `def.skills`, `<target>` the living slot index or `-1` for a single-target spec | `option=<k>` |
