...
├── iosApp/                the Xcode project — the Swift entry point hosting `ComposeUIViewController`, and `PerfTests`, an XCUITest target whose `XCTMetric`s assert the iPhone's frame and memory budgets (§ T9.5; P5)
...
- **Every workflow runs on every pull request; jobs skip themselves.** A required check
  ...
  and an `if:` that ORs its own booleans — the module booleans of the tasks it runs; `art` (with `assets`, `engine`, `ui` and `tools`) on the asset gate, the calibration golden, its second reader and `art rulers`; `engine`, `ui`, `core` and `tools` on the screenshot, frame, `vfx` and `backdrops` goldens, which render `:engine`'s rig from `:tools:instruments`; `env` on `env-check` — with `build`, `root`, `spec` and `assets`, so a
...
- `prototype/tools/capture.mjs sheets frames=1` (the tool gains `frames=1` — `sheets` reads `actor=` alone today — and `out=<path>`, resolved against the repository root — its `OUT` is hard-coded against the working directory today): writes every actor's fifteen bakes as
...
- `prototype/tools/export-study.mjs`: a Node rasteriser over the study's grid strings and
  ...
  `ember-study-x4.png` at ×4 nearest, with the sidecar of § T10.9 — its `element: FIRE` (EMBER's, from `CHARACTERS`), `pose: idle` and `frame: 0` written by the exporter, since `PixelStudy` carries none of them.
...
`frames --icon` all read (the prototype's one `seat=all|<id>` flag against the Kotlin's
...
| Reference iPhone 12 class | MED | frame p95 ≤ 12 ms; peak resident set ≤ 250 MB | L4 XCTest metrics (`iosApp/PerfTests`) |
...
- **Accounts and testers (P0)**: the Apple Developer Program and the Google Play developer
  ...
  sixteen on Android 10 or newer — 12 or newer preferred, each tester's version recorded at recruitment, since D11's fallback at P5 drops any tester below 12 — for Play (four over the rule's twelve, the target and not the gate — the fourteen days start at twelve or more opted in — the surplus of the pool a reserve that opts in before a dropout takes the count to twelve, since a dip resets the clock), at least three on iPhone with iOS 16 or newer, versions recorded at recruitment, for
...
- **Store set-up (P5, before the first device build ships to testers)**: the App Store name,
  ...
  (38 weeks at one session a week, about 50 with the contingent items, longer below it), and if any limit shorter than a year
...
except spike 6's `setup.sh` and `env.Dockerfile`, created in `ci/env/` in place, and its `ci/mint-token.sh` beside them (§ T4.1),
