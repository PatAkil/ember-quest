...
├── tools/art/             the art tool (TypeScript, Node; its own package.json and lockfile, `npm ci` in the recipe once the directory exists — the step is conditional, since spike 6 proves the recipe before the tool is built —, in Renovate's scope): generate · normalise · gate · rulers · sheet · accept — every image measurement
...
- optionally, under question 3(c): the three battle-screen defects (two root causes) fixed in
  ...
  request under the prototype's own gates and a `playfull seed=N` capture before and after,
  its decisions replayed headlessly to show the committed option and the fired skill agree
  (an unverifiable fix is reverted before the baseline play);
...
`data:` table, bound by its generated table-driven test alone and exempt, since the table is
...
`{ground: <hex>, biomes: list<object:BiomeSeats>}` with `BiomeSeats` = `biome: string` (checked by spec-lint against the biome ids' `data:` table, since this file is read outside the binder),
...
reading by reading, the renderer offsets recorded in the register and closed by the rig
before either count bar is held), not over the
...
- **Accounts and testers (P0)**: the Apple Developer Program and the Google Play developer
  ...
  sixteen on Android 10 or newer — Android 12 or newer if D11's fallback is taken — for Play (four over the rule's twelve, the surplus of the pool a reserve that opts in before a dropout takes the count to twelve, since a dip resets the clock), at least three on iPhone with iOS 16 or newer, versions recorded at recruitment, for
