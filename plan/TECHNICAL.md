...
strong-party battle fixture (the harness's three kindled relics on the fixture party — the row's `relics` are `spec/fixtures/golden/vault/default.json`'s three, read from that file, not a second copy) for
...
against a tree another agent is editing — the lesson the prototype's prototype/STATUS.md records.
...
except spike 6's `setup.sh` and `env.Dockerfile`, created in `ci/env/` in place (§ T4.1),
