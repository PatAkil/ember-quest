...
| `:storyboard` | `:ui`, `:engine`, `:core`, `:core-testing` | platform APIs except through `:engine`'s `expect` surface — the storyboard's tap-and-assert driver in its `commonMain` (`runComposeUiTest`), run on the device and the simulator by its own `androidInstrumentedTest` and `iosSimulatorArm64Test` source sets in L4; a shipped module cannot depend on a test source set and a JVM-only one can drive neither an instrumentation APK nor a simulator test, so the driver is main code in a module of its own |
...
`runSteps`, `answerWith`, `createBattle`, `nextReady`, `runTurn`, `isOver`, `battleOutcome` and `Battle.events`, so
...
and the ratio — 72 rows a set, 288 in all), the three flat sets the input of the
...
| A developer account that cannot be opened, or whose verification is delayed or refused | both enrolments started right after the go; the name reservation blocked until Apple's membership is active, the fallback name ready; a refusal stops the programme at P1 (README risks) | an enrolment unresolved fourteen days after submission |
