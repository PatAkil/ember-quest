...
- `prototype/tools/capture.mjs sheets frames=1` (the tool gains `out=<path>`, resolved against the repository root — its `OUT` is hard-coded against the working directory today): writes every actor's fifteen bakes as
...
- optionally, under question 3(c): the three battle-screen defects (two root causes) fixed in
  ...
  greying); and a `strong=`-style seeded Vault with three VIOLENT pieces equipped (three is the Vault's
...
block of `spec/golden/fixtures.md`, which `prototype/sim/fixtures.mjs` reads at run time (resolving `spec/` against the repository root it finds above `prototype/`, whatever the working directory)
...
frames and HIGH's diorama (per set: biome, seat, strip, the actor's and the strip's medians
...
- **`RULES_VERSION`** is an explicit constant in `:core`'s `version` package, bumped by the
  ...
  re-recording or a trace-format change. A corpus of saves under `spec/fixtures/save/v<N>/` (the `save` area's stem)
...
- **Store set-up (P5, before the first device build ships to testers)**: the App Store name,
  ...
  Connect validates dimensions at upload: 2868 × 1320 for the 6.9″ class and 2688 × 1242
  ...
  TestFlight's **internal** group — the owner's own install path, and all the first tag does (three to five of the lined-up testers join the internal track on that first build, three to five days before the last scheduled stop, which is taken after they have played — before the store set-up, README's P5 row),
...
- `SessionStart`: run `ci/env/setup.sh` where the host is not the image (idempotent,
  ...
  no-op), mint the App's installation token into `EMBER_APP_TOKEN` (the variable `art generate` reads, § T10.1) and confirm it.
...
(P1's M7 — the Android and iOS compile jobs, one owner ruleset act inside P1's eight sittings; the wasm job is informative until P8 — 5b's arm64 leg at M4, generated with the merge lane's jobs when the spike passed and inside that act; P2's `diff-oracle` and golden-replay jobs, one act on the P2 owner row; P3's emulator hash job — the x86-64 emulator with KVM the gate needs, which M7's compile jobs do not carry — one act on the P3 owner row; P5's device jobs; P8's wasm) — the pull request that adds the job fails
...
request is open (at M4 the art pull requests held open since P0's exit merge first, then
