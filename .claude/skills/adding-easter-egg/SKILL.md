...
Easter eggs combine two engine features: **palette swaps** (`engine/palette.ts`) and the **input module** (`engine/input.ts`). Import only from the barrel (`'../engine'`). Reference game: this game's `game/main.ts` and `game/sprites.ts` (the template patterns are already followed there). After every edit: `npm run check` (repo root).
